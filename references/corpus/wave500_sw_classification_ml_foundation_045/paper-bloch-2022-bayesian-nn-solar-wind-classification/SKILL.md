---
name: paper-bloch-2022-bayesian-nn-solar-wind-classification
description: >-
  Use when applying Bayesian neural networks to 1-au solar-wind regime classification with epistemic uncertainty — central paper claim is
  A Bayesian-NN classifier emits per-class probabilities + epistemic uncertainty consistent with class-frequency priors (TODO verify 2022).
version: 0.1.0
kind: paper-skill
quality: stub
...
paper:
  title: "Bayesian-NN variant for solar-wind classification with epistemic-uncertainty estimates"
  first_author: "TODO verify (Bloch et al. earlier draft)"
...
domain:
  ...
  secondary_themes: ["machine-learning", "bayesian-deep-learning", "1au"]
  missions: ["ACE"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["solar-wind", "classification", "bayesian", "neural-network", "epistemic-uncertainty"]
...
claim_boundary:
  scope: >-
    Stub: A Bayesian-NN classifier emits per-class probabilities + epistemic uncertainty consistent with class-frequency priors. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# Bayesian-NN variant for solar-wind classification with epistemic-uncertainty est — paper-skill (stub)

> Compiled from TODO verify (Bloch et al. earlier draft) (2022), "Bayesian-NN variant for solar-wind classification with epistemic-uncertainty estimates", TODO verify, TODO verify arXiv ID.
...
- Applying Bayesian neural networks to 1-au solar-wind regime classification with epistemic uncertainty.
...
**Claim (narrow form).** A Bayesian-NN classifier emits per-class probabilities + epistemic uncertainty consistent with class-frequency priors. The narrow form below should be tightened once the full text is verified:
...
Hypothetical earlier draft / preprint; full citation TODO verify against the eventual Bloch et al. publication record.
