---
name: paper-nguyen-2018-ml-icme-detection-svm
description: >-
  Use when running an SVM ICME detector on Wind/ACE 1-au data as a baseline vs U-Net successors — central paper claim is
  An SVM baseline detects ICMEs on Wind/ACE with TSS ≈ TODO verify (TODO verify 2018).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["event-detection", "icme", "svm", "classical-baseline"]
  missions: ["Wind", "ACE"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["icme", "svm", "event-detection", "classical-baseline", "wind", "ace"]
...
claim_boundary:
  scope: >-
    Stub: An SVM baseline detects ICMEs on Wind/ACE with TSS ≈ TODO verify. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# Support-vector-machine classical ICME detection baseline at 1 au — paper-skill (stub)

> Compiled from TODO verify (Nguyen / ML-ICME-baseline lineage) (2018), "Support-vector-machine classical ICME detection baseline at 1 au", TODO verify, TODO verify arXiv ID.
...
- Running an SVM ICME detector on Wind/ACE 1-au data as a baseline vs U-Net successors.
...
**Claim (narrow form).** An SVM baseline detects ICMEs on Wind/ACE with TSS ≈ TODO verify. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: An SVM baseline detects ICMEs on Wind/ACE with TSS ≈ TODO verify. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
Stub anchor for the SVM-baseline lineage; primary author and venue TODO verify.
