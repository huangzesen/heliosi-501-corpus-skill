---
name: paper-roy-2025-suryabench-flare-forecast-benchmark
description: >-
  Use when running the SuryaBench flare-forecast sub-task as an ML benchmark — central paper claim is
  SuryaBench publishes a labelled flare-forecast benchmark on preprocessed SDO + GOES classes (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
paper:
  title: "SuryaBench — solar flare forecasting benchmark task"
  first_author: "S. Roy"
...
domain:
  ...
  secondary_themes: ["benchmark", "dataset", "flare-forecast"]
  missions: ["SDO", "GOES"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["suryabench", "flare-forecast", "benchmark", "dataset", "goes"]
...
claim_boundary:
  scope: >-
    Stub: SuryaBench publishes a labelled flare-forecast benchmark on preprocessed SDO + GOES classes. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# SuryaBench — solar flare forecasting benchmark task — paper-skill (stub)

> Compiled from S. Roy (2025), "SuryaBench — solar flare forecasting benchmark task", TODO verify, arXiv:2508.14107.
...
- Running the SuryaBench flare-forecast sub-task as an ML benchmark.
...
**Claim (narrow form).** SuryaBench publishes a labelled flare-forecast benchmark on preprocessed SDO + GOES classes. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: SuryaBench publishes a labelled flare-forecast benchmark on preprocessed SDO + GOES classes. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
Sub-benchmark slug; horizon, threshold, and TSS protocol TODO verify.
