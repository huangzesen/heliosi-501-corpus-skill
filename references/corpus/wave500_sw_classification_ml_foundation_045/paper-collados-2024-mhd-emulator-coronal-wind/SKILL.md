---
name: paper-collados-2024-mhd-emulator-coronal-wind
description: >-
  Use when training a neural network as a fast emulator for MHD coronal + solar-wind solutions — central paper claim is
  A neural emulator reproduces reference MHD coronal-wind solutions within X% over training-domain conditions (TODO verify) (TODO verify 2024).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["machine-learning", "emulator", "mhd", "coronal-wind"]
  missions: ["SDO"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["emulator", "neural-network", "mhd", "coronal-wind", "ml-physics"]
...
claim_boundary:
  scope: >-
    Stub: A neural emulator reproduces reference MHD coronal-wind solutions within X% over training-domain conditions (TODO verify). The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# Neural-network MHD emulator for coronal + solar-wind structure — paper-skill (stub)

> Compiled from TODO verify (MHD-emulator lineage) (2024), "Neural-network MHD emulator for coronal + solar-wind structure", TODO verify, TODO verify arXiv ID.
...
- Training a neural network as a fast emulator for MHD coronal + solar-wind solutions.
...
**Claim (narrow form).** A neural emulator reproduces reference MHD coronal-wind solutions within X% over training-domain conditions (TODO verify). The narrow form below should be tightened once the full text is verified:
...
- `[[paper-orbiter-fno-spherical-surrogate]]` — sibling slug; relation TODO verify in full text.

**Research-generation affordances.** Forward-pointing surface of the skill (spec §4 Layer 4):

- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-orbiter-fno-spherical-surrogate]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Hypothetical emulator anchor; bibliographic identifiers TODO verify.
