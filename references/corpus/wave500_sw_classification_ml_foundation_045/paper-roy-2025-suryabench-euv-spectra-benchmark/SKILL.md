---
name: paper-roy-2025-suryabench-euv-spectra-benchmark
description: >-
  Use when running the SuryaBench EUV-spectra forecast sub-task as an ML benchmark — central paper claim is
  SuryaBench publishes a labelled EUV-spectra forecasting benchmark anchored on preprocessed SDO + reference EUV spectra (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
paper:
  title: "SuryaBench — EUV spectra forecasting benchmark task"
  first_author: "S. Roy"
...
domain:
  ...
  secondary_themes: ["benchmark", "dataset", "euv-spectra"]
  missions: ["SDO", "TIMED-SEE"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["suryabench", "euv-spectra", "forecast", "benchmark", "dataset"]
...
claim_boundary:
  scope: >-
    Stub: SuryaBench publishes a labelled EUV-spectra forecasting benchmark anchored on preprocessed SDO + reference EUV spectra. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
depends_on: ["paper-roy-2025-suryabench-ml-benchmark-dataset"]
adapter_notes: []
research_generation_affordances:
  - type: gap
    statement: "Stub skill — methods, data contracts, and validation target not yet compiled from the primary source."
    related_skills: ["paper-roy-2025-suryabench-ml-benchmark-dataset"]
...
# SuryaBench — EUV spectra forecasting benchmark task — paper-skill (stub)

> Compiled from S. Roy (2025), "SuryaBench — EUV spectra forecasting benchmark task", TODO verify, arXiv:2508.14107.
...
- Running the SuryaBench EUV-spectra forecast sub-task as an ML benchmark.
...
**Claim (narrow form).** SuryaBench publishes a labelled EUV-spectra forecasting benchmark anchored on preprocessed SDO + reference EUV spectra. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: SuryaBench publishes a labelled EUV-spectra forecasting benchmark anchored on preprocessed SDO + reference EUV spectra. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-roy-2025-suryabench-ml-benchmark-dataset]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Sub-benchmark slug; reference EUV spectrum dataset / metric TODO verify.
