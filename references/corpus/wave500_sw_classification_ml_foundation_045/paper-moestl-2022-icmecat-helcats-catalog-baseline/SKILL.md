---
name: paper-moestl-2022-icmecat-helcats-catalog-baseline
description: >-
  Use when using the ICMECAT / HELCATS multi-spacecraft ICME catalog as a ground-truth label source for ML pipelines — central paper claim is
  ICMECAT / HELCATS catalogs provide multi-spacecraft start/end labels supporting ICME detector training and validation (TODO verify (Space Weather; or HELCATS dataset paper) 2022).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["event-detection", "icme", "catalog", "dataset"]
  missions: ["Wind", "STEREO", "ACE", "MESSENGER", "VEX", "Maven"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["icmecat", "helcats", "icme", "catalog", "training-set", "multi-spacecraft"]
...
claim_boundary:
  scope: >-
    Stub: ICMECAT / HELCATS catalogs provide multi-spacecraft start/end labels supporting ICME detector training and validation. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# ICMECAT / HELCATS ICME catalog as ML training-set baseline — paper-skill (stub)

> Compiled from C. Möstl (2022), "ICMECAT / HELCATS ICME catalog as ML training-set baseline", TODO verify (Space Weather; or HELCATS dataset paper), TODO verify arXiv ID.
...
- Using the ICMECAT / HELCATS multi-spacecraft ICME catalog as a ground-truth label source for ML pipelines.
...
**Claim (narrow form).** ICMECAT / HELCATS catalogs provide multi-spacecraft start/end labels supporting ICME detector training and validation. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: ICMECAT / HELCATS catalogs provide multi-spacecraft start/end labels supporting ICME detector training and validation. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
- `[[paper-rudisser-2022-icme-unet-automatic-detection]]` — sibling slug; relation TODO verify in full text.

**Research-generation affordances.** Forward-pointing surface of the skill (spec §4 Layer 4):

- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-rudisser-2022-icme-unet-automatic-detection]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Stub anchor for the canonical training-label catalog; exact catalog version / DOI TODO verify.
