---
name: paper-roy-2025-surya-flare-forecast-finetune
description: >-
  Use when fine-tuning Surya on a flare-forecast classification target (M+ / X+ class within a horizon) — central paper claim is
  LoRA fine-tuning of Surya yields competitive flare-forecast skill scores on a held-out test cycle (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
paper:
  title: "Surya foundation-model fine-tune for solar flare forecasting"
  first_author: "S. Roy"
...
domain:
  ...
  secondary_themes: ["foundation-model", "flare-forecast", "lora", "downstream-task"]
  missions: ["SDO", "GOES"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["surya", "foundation-model", "flare-forecast", "lora", "sdo", "goes"]
...
claim_boundary:
  scope: >-
    Stub: LoRA fine-tuning of Surya yields competitive flare-forecast skill scores on a held-out test cycle. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# Surya foundation-model fine-tune for solar flare forecasting — paper-skill (stub)

> Compiled from S. Roy (2025), "Surya foundation-model fine-tune for solar flare forecasting", TODO verify, arXiv:2508.14112.
...
- Fine-tuning Surya on a flare-forecast classification target (M+ / X+ class within a horizon).
...
**Claim (narrow form).** LoRA fine-tuning of Surya yields competitive flare-forecast skill scores on a held-out test cycle. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: LoRA fine-tuning of Surya yields competitive flare-forecast skill scores on a held-out test cycle. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
Captures the flare-forecast downstream task; horizon, threshold, and TSS TODO verify in the full paper or technical report.
