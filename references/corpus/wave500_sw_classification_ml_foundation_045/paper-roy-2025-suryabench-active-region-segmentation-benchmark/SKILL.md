---
name: paper-roy-2025-suryabench-active-region-segmentation-benchmark
description: >-
  Use when running the SuryaBench active-region-segmentation sub-task as an ML benchmark — central paper claim is
  SuryaBench publishes a labelled active-region-segmentation benchmark on preprocessed SDO AIA/HMI (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
paper:
  title: "SuryaBench — active-region segmentation benchmark task"
  first_author: "S. Roy"
...
domain:
  ...
  secondary_themes: ["benchmark", "dataset", "active-region-segmentation"]
  missions: ["SDO"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["suryabench", "active-region", "segmentation", "benchmark", "dataset", "sdo"]
...
claim_boundary:
  scope: >-
    Stub: SuryaBench publishes a labelled active-region-segmentation benchmark on preprocessed SDO AIA/HMI. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# SuryaBench — active-region segmentation benchmark task — paper-skill (stub)

> Compiled from S. Roy (2025), "SuryaBench — active-region segmentation benchmark task", TODO verify, arXiv:2508.14107.
...
- Running the SuryaBench active-region-segmentation sub-task as an ML benchmark.
...
**Claim (narrow form).** SuryaBench publishes a labelled active-region-segmentation benchmark on preprocessed SDO AIA/HMI. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: SuryaBench publishes a labelled active-region-segmentation benchmark on preprocessed SDO AIA/HMI. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
Sub-benchmark slug; exact label source and IoU evaluation protocol TODO verify.
