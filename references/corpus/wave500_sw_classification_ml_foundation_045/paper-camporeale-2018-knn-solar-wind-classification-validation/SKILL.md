---
name: paper-camporeale-2018-knn-solar-wind-classification-validation
description: >-
  Use when validating supervised k-NN-style solar-wind classifiers against the Xu–Borovsky 4-class scheme at 1 au — central paper claim is
  Cross-validated k-NN tops a 10-model benchmark at ≈92.8% accuracy on the Xu–Borovsky 4-class labelling (TODO verify (J. Geophys. Res. Space Phys. or arXiv:1811.02323 successor) 2018).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["machine-learning", "supervised-classification", "1au"]
  missions: ["ACE", "Wind"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["solar-wind", "classification", "knn", "1au", "ace", "xu-borovsky", "supervised", "ml-benchmark"]
...
claim_boundary:
  scope: >-
    Stub: Cross-validated k-NN tops a 10-model benchmark at ≈92.8% accuracy on the Xu–Borovsky 4-class labelling. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
depends_on: ["paper-camporeale-2017-knn-solar-wind-categorization", "paper-bloch-2024-uncertainty-nn-solar-wind-types"]
adapter_notes: []
research_generation_affordances:
  - type: gap
    statement: "Stub skill — methods, data contracts, and validation target not yet compiled from the primary source."
    related_skills: ["paper-camporeale-2017-knn-solar-wind-categorization", "paper-bloch-2024-uncertainty-nn-solar-wind-types"]
...
# Cross-validation extension of supervised k-NN solar wind classification at 1 au — paper-skill (stub)

> Compiled from E. Camporeale (2018), "Cross-validation extension of supervised k-NN solar wind classification at 1 au", TODO verify (J. Geophys. Res. Space Phys. or arXiv:1811.02323 successor), arXiv:1811.02323.
...
- Validating supervised k-NN-style solar-wind classifiers against the Xu–Borovsky 4-class scheme at 1 au.
...
**Claim (narrow form).** Cross-validated k-NN tops a 10-model benchmark at ≈92.8% accuracy on the Xu–Borovsky 4-class labelling. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: Cross-validated k-NN tops a 10-model benchmark at ≈92.8% accuracy on the Xu–Borovsky 4-class labelling. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
- `[[paper-camporeale-2017-knn-solar-wind-categorization]]` — sibling slug; relation TODO verify in full text.
- `[[paper-bloch-2024-uncertainty-nn-solar-wind-types]]` — sibling slug; relation TODO verify in full text.

**Research-generation affordances.** Forward-pointing surface of the skill (spec §4 Layer 4):

- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-camporeale-2017-knn-solar-wind-categorization]]`, `[[paper-bloch-2024-uncertainty-nn-solar-wind-types]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Stub built from extended_search.md §6.1/§6.5 abstracts only; full-text feature list, train/test split, and per-class confusion matrix are TODO verify.
