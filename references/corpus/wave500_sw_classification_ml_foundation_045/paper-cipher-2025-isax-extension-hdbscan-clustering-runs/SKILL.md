---
name: paper-cipher-2025-isax-extension-hdbscan-clustering-runs
description: >-
  Use when evaluating reproducibility of CIPHER iSAX + HDBSCAN cluster solutions across hyperparameter / seed sweeps — central paper claim is
  CIPHER cluster solutions are reproducible within ARI ≥ TODO verify across iSAX cardinality / HDBSCAN min-cluster-size sweeps (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
paper:
  title: "CIPHER iSAX+HDBSCAN extension — clustering-run reproducibility study"
  first_author: "TODO verify (CIPHER authors)"
...
domain:
  ...
  secondary_themes: ["time-series", "symbolic-representation", "hdbscan", "human-in-the-loop"]
  missions: ["PSP", "Wind"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["cipher", "isax", "hdbscan", "clustering", "reproducibility", "solar-wind"]
...
links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2510.21022"
  ads_url: null
  code_repo: null
  data_repo: null
claim_boundary:
  scope: >-
    Stub: CIPHER cluster solutions are reproducible within ARI ≥ TODO verify across iSAX cardinality / HDBSCAN min-cluster-size sweeps. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
depends_on: ["paper-cipher-2025-isax-hdbscan-solar-wind-segmentation"]
adapter_notes: []
research_generation_affordances:
  - type: gap
    statement: "Stub skill — methods, data contracts, and validation target not yet compiled from the primary source."
    related_skills: ["paper-cipher-2025-isax-hdbscan-solar-wind-segmentation"]
...
- Evaluating reproducibility of CIPHER iSAX + HDBSCAN cluster solutions across hyperparameter / seed sweeps.
...
**Claim (narrow form).** CIPHER cluster solutions are reproducible within ARI ≥ TODO verify across iSAX cardinality / HDBSCAN min-cluster-size sweeps. The narrow form below should be tightened once the full text is verified:
...
- `[[paper-cipher-2025-isax-hdbscan-solar-wind-segmentation]]` — sibling slug; relation TODO verify in full text.

**Research-generation affordances.** Forward-pointing surface of the skill (spec §4 Layer 4):

- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-cipher-2025-isax-hdbscan-solar-wind-segmentation]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Hypothetical companion analysis; bibliographic anchor TODO verify.
