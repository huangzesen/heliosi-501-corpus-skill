---
name: paper-li-2025-tianwen1-transformer-mag-calibration
description: >-
  Use when calibrating spacecraft-magnetometer measurements with a transformer model constrained by Maxwell's equations — central paper claim is
  A physics-informed transformer corrects spacecraft-magnetometer anomalies and dramatically shortens calibration cycle time (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["machine-learning", "transformer", "physics-informed", "calibration", "mars"]
  missions: ["Tianwen-1"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["transformer", "physics-informed", "calibration", "magnetometer", "tianwen-1", "mars", "maxwell"]
...
links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2501.00020"
  ads_url: null
  code_repo: null
  data_repo: null
claim_boundary:
  scope: >-
    Stub: A physics-informed transformer corrects spacecraft-magnetometer anomalies and dramatically shortens calibration cycle time. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# Transformer with Maxwell-equation physical constraints for Tianwen-1 magnetomete — paper-skill (stub)

> Compiled from B. Li (2025), "Transformer with Maxwell-equation physical constraints for Tianwen-1 magnetometer calibration", TODO verify, arXiv:2501.00020.
...
- Calibrating spacecraft-magnetometer measurements with a transformer model constrained by Maxwell's equations.
...
**Claim (narrow form).** A physics-informed transformer corrects spacecraft-magnetometer anomalies and dramatically shortens calibration cycle time. The narrow form below should be tightened once the full text is verified:
...
- arXiv: https://arxiv.org/abs/2501.00020
...
- `[[paper-cdflib-cdf-reader]]` — sibling slug; relation TODO verify in full text.

**Research-generation affordances.** Forward-pointing surface of the skill (spec §4 Layer 4):

- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-cdflib-cdf-reader]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Stub built from theme_solar_wind_segmentation.json entry arxiv_id=2501.00020. Calibration error metric and training-set definition TODO verify.
