---
name: paper-jarolim-2023-coronal-hole-acwe-consistency
description: >-
  Use when quantifying the consistency / confidence of ACWE-based coronal-hole detections across input parameter sweeps — central paper claim is
  ACWE coronal-hole boundaries are reproducible within X% IoU across hyperparameter sweeps (TODO verify) (TODO verify 2023).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["image-segmentation", "coronal-hole", "acwe", "uncertainty-quantification"]
  missions: ["SDO"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["acwe", "coronal-hole", "segmentation", "consistency", "confidence", "uncertainty-quantification"]
...
links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2308.05679"
  ads_url: null
  code_repo: null
  data_repo: null
claim_boundary:
  scope: >-
    Stub: ACWE coronal-hole boundaries are reproducible within X% IoU across hyperparameter sweeps (TODO verify). The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
depends_on: ["paper-grajeda-2025-acwe-magnetic-constrained-ch-segmentation"]
adapter_notes: []
research_generation_affordances:
  - type: gap
    statement: "Stub skill — methods, data contracts, and validation target not yet compiled from the primary source."
    related_skills: ["paper-grajeda-2025-acwe-magnetic-constrained-ch-segmentation"]
...
# Consistency and confidence quantification of ACWE coronal-hole segmentation — paper-skill (stub)

> Compiled from TODO verify (Jarolim / consistency study lineage) (2023), "Consistency and confidence quantification of ACWE coronal-hole segmentation", TODO verify, arXiv:2308.05679.
...
- Quantifying the consistency / confidence of ACWE-based coronal-hole detections across input parameter sweeps.
...
**Claim (narrow form).** ACWE coronal-hole boundaries are reproducible within X% IoU across hyperparameter sweeps (TODO verify). The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: ACWE coronal-hole boundaries are reproducible within X% IoU across hyperparameter sweeps (TODO verify). The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
- arXiv: https://arxiv.org/abs/2308.05679
...
- `[[paper-grajeda-2025-acwe-magnetic-constrained-ch-segmentation]]` — sibling slug; relation TODO verify in full text.

**Research-generation affordances.** Forward-pointing surface of the skill (spec §4 Layer 4):

- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-grajeda-2025-acwe-magnetic-constrained-ch-segmentation]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Stub built from theme_solar_wind_segmentation.json entry arxiv_id=2308.05679. Author list and IoU statistics TODO verify.
