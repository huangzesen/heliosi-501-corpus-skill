---
name: paper-camporeale-2020-icme-classification-ml-benchmark
description: >-
  Use when benchmarking ML classifiers (RF, GBM, NN, transformer) on ICME vs sheath vs ambient labelling — central paper claim is
  A cross-mission benchmark ranks ICME classifiers by TSS / HSS, with deep models leading on long-window inputs (TODO verify 2020).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["event-detection", "icme", "classification", "ml-benchmark"]
  missions: ["Wind", "ACE", "STEREO"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["icme", "sheath", "classification", "ml-benchmark", "tss", "hss"]
...
claim_boundary:
  scope: >-
    Stub: A cross-mission benchmark ranks ICME classifiers by TSS / HSS, with deep models leading on long-window inputs. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
- Benchmarking ML classifiers (RF, GBM, NN, transformer) on ICME vs sheath vs ambient labelling.
...
**Claim (narrow form).** A cross-mission benchmark ranks ICME classifiers by TSS / HSS, with deep models leading on long-window inputs. The narrow form below should be tightened once the full text is verified:
...
Hypothetical benchmark anchor; bibliographic identifiers TODO verify.
