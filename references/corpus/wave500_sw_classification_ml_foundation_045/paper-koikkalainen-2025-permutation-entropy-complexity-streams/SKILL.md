---
name: paper-koikkalainen-2025-permutation-entropy-complexity-streams
description: >-
  Use when isolating the permutation-entropy axis of the Koikkalainen-2025 complexity-plane discrimination of fast/slow/MC/sheath streams — central paper claim is
  Permutation entropy alone separates MC-driven from ambient streams above an SNR / window-length threshold (TODO verify) (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["complexity", "permutation-entropy", "information-theory", "stream-classification"]
  missions: ["Wind", "ACE"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["permutation-entropy", "complexity", "solar-wind", "stream", "magnetic-cloud", "ml-feature"]
...
links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2510.05873"
  ads_url: null
  code_repo: null
  data_repo: null
claim_boundary:
  scope: >-
    Stub: Permutation entropy alone separates MC-driven from ambient streams above an SNR / window-length threshold (TODO verify). The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
depends_on: ["paper-koikkalainen-2025-complexity-solar-wind-streams"]
adapter_notes: []
research_generation_affordances:
  - type: gap
    statement: "Stub skill — methods, data contracts, and validation target not yet compiled from the primary source."
    related_skills: ["paper-koikkalainen-2025-complexity-solar-wind-streams"]
...
# Permutation-entropy variant of Koikkalainen-2025 complexity analysis of solar-wi — paper-skill (stub)

> Compiled from V. Koikkalainen (2025), "Permutation-entropy variant of Koikkalainen-2025 complexity analysis of solar-wind streams", TODO verify, arXiv:2510.05873.
...
- Isolating the permutation-entropy axis of the Koikkalainen-2025 complexity-plane discrimination of fast/slow/MC/sheath streams.
...
**Claim (narrow form).** Permutation entropy alone separates MC-driven from ambient streams above an SNR / window-length threshold (TODO verify). The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: Permutation entropy alone separates MC-driven from ambient streams above an SNR / window-length threshold (TODO verify). The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
- `[[paper-koikkalainen-2025-complexity-solar-wind-streams]]` — sibling slug; relation TODO verify in full text.

**Research-generation affordances.** Forward-pointing surface of the skill (spec §4 Layer 4):

- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-koikkalainen-2025-complexity-solar-wind-streams]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Captures the permutation-entropy sub-claim of the parent complexity-plane paper-skill; exact thresholds TODO verify.
