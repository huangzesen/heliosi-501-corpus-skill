---
name: paper-stansby-2025-flux-tube-segmentation
description: >-
  Use when segmenting solar-wind streams into flux-tube populations based on B-field topology — central paper claim is
  Flux-tube segmentation yields a population catalog whose occurrence statistics correlate with source-region heuristics (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["segmentation", "flux-tube", "topology", "stream-structure"]
  missions: ["Wind", "ACE", "SolO"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["flux-tube", "topology", "segmentation", "solar-wind", "stream-structure"]
...
claim_boundary:
  scope: >-
    Stub: Flux-tube segmentation yields a population catalog whose occurrence statistics correlate with source-region heuristics. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# Flux-tube segmentation of solar-wind streams from B-field topology — paper-skill (stub)

> Compiled from D. Stansby (extension lineage) (2025), "Flux-tube segmentation of solar-wind streams from B-field topology", TODO verify, TODO verify arXiv ID.
...
- Segmenting solar-wind streams into flux-tube populations based on B-field topology.
...
**Claim (narrow form).** Flux-tube segmentation yields a population catalog whose occurrence statistics correlate with source-region heuristics. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: Flux-tube segmentation yields a population catalog whose occurrence statistics correlate with source-region heuristics. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-stansby-2020-pfsspy-python-pfss]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Hypothetical extension; bibliographic anchor TODO verify.
