---
name: paper-uritsky-2025-qraft-open-flux-segmentation
description: >-
  Use when segmenting open-flux coronal features from coronagraph images via quasi-radial field-line tracing — central paper claim is
  QRaFT extracts field-aligned optical coronal features within ~4–7° of the local magnetic field in MHD synthetic tests (TODO verify (likely ApJ) 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["image-segmentation", "open-flux", "coronagraph", "field-line-tracing"]
  missions: ["SOHO", "ground-based-eclipse"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["qraft", "open-flux", "segmentation", "coronagraph", "field-line-tracing", "mhd-validation"]
...
# QRaFT — adaptive segmentation of the open-flux solar corona — paper-skill (stub)

> Compiled from V. M. Uritsky (2025), "QRaFT — adaptive segmentation of the open-flux solar corona", TODO verify (likely ApJ), arXiv:2506.14894.
...
- Segmenting open-flux coronal features from coronagraph images via quasi-radial field-line tracing.
...
**Claim (narrow form).** QRaFT extracts field-aligned optical coronal features within ~4–7° of the local magnetic field in MHD synthetic tests. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: QRaFT extracts field-aligned optical coronal features within ~4–7° of the local magnetic field in MHD synthetic tests. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
Stub built from theme_solar_wind_segmentation.json entry arxiv_id=2506.14894. Reference MHD model details and the real-image performance metrics TODO verify in the full paper.
