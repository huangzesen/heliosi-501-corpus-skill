---
name: paper-bizoulasso-2025-pop-corn-neural-ch-validation
description: >-
  Use when validating a neural-network coronal-hole detection tool against ACWE / human-labelled references — central paper claim is
  POP-CORN matches ACWE / human CH masks with IoU > X% across SDO / STEREO / PROBA-2 (TODO verify) (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["image-segmentation", "coronal-hole", "neural-network", "validation"]
  missions: ["SDO", "STEREO", "PROBA-2"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["pop-corn", "coronal-hole", "neural-network", "validation", "segmentation"]
...
links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2603.25591"
  ads_url: null
  code_repo: null
  data_repo: null
claim_boundary:
  scope: >-
    Stub: POP-CORN matches ACWE / human CH masks with IoU > X% across SDO / STEREO / PROBA-2 (TODO verify). The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
- Validating a neural-network coronal-hole detection tool against ACWE / human-labelled references.
...
**Claim (narrow form).** POP-CORN matches ACWE / human CH masks with IoU > X% across SDO / STEREO / PROBA-2 (TODO verify). The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: POP-CORN matches ACWE / human CH masks with IoU > X% across SDO / STEREO / PROBA-2 (TODO verify). The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
- arXiv: https://arxiv.org/abs/2603.25591
...
Stub built from theme_solar_wind_segmentation.json entry arxiv_id=2603.25591. Author list and IoU thresholds TODO verify.
