---
name: paper-trotta-2025-shock-detection-multispacecraft-ml
description: >-
  Use when augmenting multi-spacecraft IP-shock catalogs with ML-assisted shock-crossing detection — central paper claim is
  An ML-assisted detector recovers TODO verify% of Trotta-2025 multi-spacecraft shocks with FPR ≤ TODO verify (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["event-detection", "shock", "interplanetary-shock", "ml-assist"]
  missions: ["SolO", "PSP", "Wind", "STEREO"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["interplanetary-shock", "multi-spacecraft", "event-detection", "ml-assist"]
...
claim_boundary:
  scope: >-
    Stub: An ML-assisted detector recovers TODO verify% of Trotta-2025 multi-spacecraft shocks with FPR ≤ TODO verify. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
depends_on: ["paper-trotta-2025-ip-shock-variability-multi-spacecraft"]
adapter_notes: []
...
# ML-assisted multi-spacecraft IP-shock detection extending Trotta 2025 — paper-skill (stub)

> Compiled from D. Trotta (extension lineage) (2025), "ML-assisted multi-spacecraft IP-shock detection extending Trotta 2025", TODO verify, TODO verify arXiv ID.
...
- Augmenting multi-spacecraft IP-shock catalogs with ML-assisted shock-crossing detection.
...
**Claim (narrow form).** An ML-assisted detector recovers TODO verify% of Trotta-2025 multi-spacecraft shocks with FPR ≤ TODO verify. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: An ML-assisted detector recovers TODO verify% of Trotta-2025 multi-spacecraft shocks with FPR ≤ TODO verify. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
- `[[paper-trotta-2025-ip-shock-variability-multi-spacecraft]]` — sibling slug; relation TODO verify in full text.

**Research-generation affordances.** Forward-pointing surface of the skill (spec §4 Layer 4):

- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-trotta-2025-ip-shock-variability-multi-spacecraft]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Hypothetical companion; bibliographic anchor TODO verify.
