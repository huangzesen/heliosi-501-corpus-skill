---
name: paper-pal-2024-cir-stream-interaction-region-ml
description: >-
  Use when training ML segmentation models to mark CIR / SIR boundaries on 1-au in-situ time series — central paper claim is
  ML segmentation recovers CIR / SIR boundary catalogs with TSS ≈ TODO verify vs manual catalogs (TODO verify 2024).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["event-detection", "cir", "stream-interaction", "machine-learning"]
  missions: ["ACE", "Wind", "STEREO"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["cir", "stream-interaction-region", "event-detection", "ml", "segmentation", "1au"]
...
claim_boundary:
  scope: >-
    Stub: ML segmentation recovers CIR / SIR boundary catalogs with TSS ≈ TODO verify vs manual catalogs. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# ML detection of CIR / stream-interaction-region boundaries at 1 au — paper-skill (stub)

> Compiled from TODO verify (Pal / CIR-ML lineage) (2024), "ML detection of CIR / stream-interaction-region boundaries at 1 au", TODO verify, TODO verify arXiv ID.
...
- Training ML segmentation models to mark CIR / SIR boundaries on 1-au in-situ time series.
...
**Claim (narrow form).** ML segmentation recovers CIR / SIR boundary catalogs with TSS ≈ TODO verify vs manual catalogs. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: ML segmentation recovers CIR / SIR boundary catalogs with TSS ≈ TODO verify vs manual catalogs. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
Stub anchor; primary author and venue TODO verify.
