---
name: paper-bloch-2024-uncertainty-nn-extension-1au
description: >-
  Use when extending uncertainty-aware NN solar-wind classifiers with new calibration / reliability diagrams — central paper claim is
  Calibrated-uncertainty NN classification preserves 4-class accuracy while emitting reliable per-prediction confidences (TODO verify 2024).
version: 0.1.0
kind: paper-skill
quality: stub
...
paper:
  title: "Calibrated-uncertainty NN extension for 1-au solar-wind class assignment"
  first_author: "TODO verify (Bloch et al. successor)"
...
domain:
  ...
  secondary_themes: ["machine-learning", "uncertainty-quantification", "neural-network", "1au"]
  missions: ["ACE", "Wind"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["solar-wind", "classification", "uncertainty", "neural-network", "1au", "calibration"]
...
claim_boundary:
  scope: >-
    Stub: Calibrated-uncertainty NN classification preserves 4-class accuracy while emitting reliable per-prediction confidences. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
depends_on: ["paper-bloch-2024-uncertainty-nn-solar-wind-types"]
adapter_notes: []
research_generation_affordances:
  - type: gap
    statement: "Stub skill — methods, data contracts, and validation target not yet compiled from the primary source."
    related_skills: ["paper-bloch-2024-uncertainty-nn-solar-wind-types"]
...
# Calibrated-uncertainty NN extension for 1-au solar-wind class assignment — paper-skill (stub)

> Compiled from TODO verify (Bloch et al. successor) (2024), "Calibrated-uncertainty NN extension for 1-au solar-wind class assignment", TODO verify, TODO verify arXiv ID.
...
- Extending uncertainty-aware NN solar-wind classifiers with new calibration / reliability diagrams.
...
**Claim (narrow form).** Calibrated-uncertainty NN classification preserves 4-class accuracy while emitting reliable per-prediction confidences. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: Calibrated-uncertainty NN classification preserves 4-class accuracy while emitting reliable per-prediction confidences. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-bloch-2024-uncertainty-nn-solar-wind-types]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Stub: complements existing Bloch 2024 paper-skill with the calibration / reliability sub-claim.
