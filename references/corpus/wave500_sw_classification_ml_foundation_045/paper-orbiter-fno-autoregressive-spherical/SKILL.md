---
name: paper-orbiter-fno-autoregressive-spherical
description: >-
  Use when autoregressive rollout of a spherical FNO for multi-step solar-wind forecasts — central paper claim is
  An autoregressive spherical FNO produces stable multi-step solar-wind forecasts beyond a single time-step (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["machine-learning", "neural-operator", "autoregressive", "solar-wind-forecast"]
  missions: ["SDO", "OMNI"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["spherical-fno", "autoregressive", "solar-wind-forecast", "neural-operator", "rollout"]
...
links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2511.20830"
  ads_url: null
  code_repo: null
  data_repo: null
claim_boundary:
  scope: >-
    Stub: An autoregressive spherical FNO produces stable multi-step solar-wind forecasts beyond a single time-step. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
- Autoregressive rollout of a spherical FNO for multi-step solar-wind forecasts.
...
**Claim (narrow form).** An autoregressive spherical FNO produces stable multi-step solar-wind forecasts beyond a single time-step. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: An autoregressive spherical FNO produces stable multi-step solar-wind forecasts beyond a single time-step. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
- arXiv: https://arxiv.org/abs/2511.20830
...
Companion to the spherical-FNO surrogate slug; rollout-stability metric (e.g., divergence over N steps) TODO verify.
