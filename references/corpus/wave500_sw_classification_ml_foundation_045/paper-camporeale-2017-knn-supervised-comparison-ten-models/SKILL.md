---
name: paper-camporeale-2017-knn-supervised-comparison-ten-models
description: >-
  Use when comparing supervised ML model families (KNN, SVM, RF, GBM, NN, …) on the Xu–Borovsky 4-class labels at 1 au — central paper claim is
  Across a 10-model benchmark, KNN tops the scoreboard at ≈92.8% accuracy on 4-class 1-au labelling (TODO verify (J. Geophys. Res. Space Phys. or companion paper) 2017).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["machine-learning", "supervised-benchmark", "model-comparison"]
  missions: ["ACE"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["solar-wind", "classification", "knn", "svm", "random-forest", "benchmark", "1au"]
...
claim_boundary:
  scope: >-
    Stub: Across a 10-model benchmark, KNN tops the scoreboard at ≈92.8% accuracy on 4-class 1-au labelling. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# Ten-model supervised benchmark comparison for solar-wind classification (Campore — paper-skill (stub)

> Compiled from E. Camporeale (2017), "Ten-model supervised benchmark comparison for solar-wind classification (Camporeale 2017 companion)", TODO verify (J. Geophys. Res. Space Phys. or companion paper), arXiv:1811.02323.
...
- Comparing supervised ML model families (KNN, SVM, RF, GBM, NN, …) on the Xu–Borovsky 4-class labels at 1 au.
...
**Claim (narrow form).** Across a 10-model benchmark, KNN tops the scoreboard at ≈92.8% accuracy on 4-class 1-au labelling. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: Across a 10-model benchmark, KNN tops the scoreboard at ≈92.8% accuracy on 4-class 1-au labelling. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-camporeale-2017-knn-solar-wind-categorization]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Distinct sub-claim from the parent slug: this skill is the model-zoo *comparison*, not the head KNN result. Companion paper / supplementary venue TODO verify.
