---
name: paper-jarolim-2024-ch-segmentation-cycle-validation
description: >-
  Use when evaluating CH segmentation methods across full Solar Cycles 23–25 for instrument-bias drifts — central paper claim is
  CH detection drifts across cycles 23–25 are bounded by X% area when normalised by instrument response (TODO verify) (TODO verify 2024).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["image-segmentation", "coronal-hole", "long-term-validation", "solar-cycle"]
  missions: ["SDO", "SOHO", "STEREO"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["coronal-hole", "segmentation", "solar-cycle", "validation", "instrument-bias"]
...
claim_boundary:
  scope: >-
    Stub: CH detection drifts across cycles 23–25 are bounded by X% area when normalised by instrument response (TODO verify). The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
- Evaluating CH segmentation methods across full Solar Cycles 23–25 for instrument-bias drifts.
...
**Claim (narrow form).** CH detection drifts across cycles 23–25 are bounded by X% area when normalised by instrument response (TODO verify). The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: CH detection drifts across cycles 23–25 are bounded by X% area when normalised by instrument response (TODO verify). The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
Hypothetical anchor; bibliographic identifiers TODO verify.
