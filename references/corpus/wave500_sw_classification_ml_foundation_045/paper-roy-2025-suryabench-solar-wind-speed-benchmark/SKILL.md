---
name: paper-roy-2025-suryabench-solar-wind-speed-benchmark
description: >-
  Use when running the SuryaBench solar-wind speed sub-task as an ML benchmark — central paper claim is
  SuryaBench publishes a labelled solar-wind speed forecasting benchmark on preprocessed SDO + OMNI (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
paper:
  title: "SuryaBench — solar-wind speed estimation benchmark task"
  first_author: "S. Roy"
...
domain:
  ...
  secondary_themes: ["benchmark", "dataset", "solar-wind-forecast"]
  missions: ["SDO", "OMNI"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["suryabench", "solar-wind-speed", "forecast", "benchmark", "dataset"]
...
claim_boundary:
  scope: >-
    Stub: SuryaBench publishes a labelled solar-wind speed forecasting benchmark on preprocessed SDO + OMNI. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# SuryaBench — solar-wind speed estimation benchmark task — paper-skill (stub)

> Compiled from S. Roy (2025), "SuryaBench — solar-wind speed estimation benchmark task", TODO verify, arXiv:2508.14107.
...
- Running the SuryaBench solar-wind speed sub-task as an ML benchmark.
...
**Claim (narrow form).** SuryaBench publishes a labelled solar-wind speed forecasting benchmark on preprocessed SDO + OMNI. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: SuryaBench publishes a labelled solar-wind speed forecasting benchmark on preprocessed SDO + OMNI. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
Sub-benchmark slug; horizon and MAE protocol TODO verify.
