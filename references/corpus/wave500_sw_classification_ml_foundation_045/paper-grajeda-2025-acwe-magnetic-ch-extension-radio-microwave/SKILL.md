---
name: paper-grajeda-2025-acwe-magnetic-ch-extension-radio-microwave
description: >-
  Use when extending the magnetic-constrained ACWE pipeline to microwave / radio coronal-hole imagery — central paper claim is
  Magnetic-constrained ACWE on microwave maps preserves IoU vs EUV ground truth on the same dates (TODO verify) (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["image-segmentation", "coronal-hole", "radio", "microwave", "acwe"]
  missions: ["Nobeyama", "ALMA", "EOVSA", "SDO"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["acwe", "coronal-hole", "microwave", "radio", "magnetic-constraint", "extension"]
...
claim_boundary:
  scope: >-
    Stub: Magnetic-constrained ACWE on microwave maps preserves IoU vs EUV ground truth on the same dates (TODO verify). The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# Microwave / radio extension of magnetic-constrained ACWE coronal-hole segmentati — paper-skill (stub)

> Compiled from J. A. Grajeda (extension lineage) (2025), "Microwave / radio extension of magnetic-constrained ACWE coronal-hole segmentation", TODO verify, TODO verify arXiv ID.
...
- Extending the magnetic-constrained ACWE pipeline to microwave / radio coronal-hole imagery.
...
**Claim (narrow form).** Magnetic-constrained ACWE on microwave maps preserves IoU vs EUV ground truth on the same dates (TODO verify). The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: Magnetic-constrained ACWE on microwave maps preserves IoU vs EUV ground truth on the same dates (TODO verify). The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
Hypothetical extension of the magnetic-constrained ACWE work; venue / IoU TODO verify.
