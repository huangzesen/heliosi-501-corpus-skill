---
name: tool-skill-sktime-time-series-classification-for-helio
description: >-
  Use when running standardised time-series classification / forecasting / clustering pipelines on heliophysics in-situ time series — central paper claim is
  sktime exposes a uniform interface to dozens of TS classifiers / forecasters / clusterers usable by helio ML pipelines (TODO verify (JMLR or sktime documentation) 2022).
version: 0.1.0
kind: tool-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["tool-skill", "time-series", "classification", "infrastructure"]
  missions: ["multi-mission"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["sktime", "time-series", "classification", "tool-skill", "infrastructure"]
...
claim_boundary:
  scope: >-
    Stub: sktime exposes a uniform interface to dozens of TS classifiers / forecasters / clusterers usable by helio ML pipelines. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
- Running standardised time-series classification / forecasting / clustering pipelines on heliophysics in-situ time series.
...
**Claim (narrow form).** sktime exposes a uniform interface to dozens of TS classifiers / forecasters / clusterers usable by helio ML pipelines. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: sktime exposes a uniform interface to dozens of TS classifiers / forecasters / clusterers usable by helio ML pipelines. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
**Skill graph (depends_on edges).** No paper-skill dependencies (self-contained at stub tier).

**Research-generation affordances.** Forward-pointing surface of the skill (spec §4 Layer 4):

- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: (none). Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Tool-skill anchor; canonical citation / latest version TODO verify before any consumer integration.
