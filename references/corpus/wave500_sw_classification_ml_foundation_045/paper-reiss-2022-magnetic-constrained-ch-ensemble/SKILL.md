---
name: paper-reiss-2022-magnetic-constrained-ch-ensemble
description: >-
  Use when running an ensemble CH-segmentation pipeline constrained by HMI / GONG photospheric magnetic-field data — central paper claim is
  A magnetic-constrained ensemble reduces filament false positives and improves CH boundary stability vs single-method baselines (TODO verify 2022).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["image-segmentation", "coronal-hole", "ensemble", "magnetic-constraint", "chromosphere"]
  missions: ["SDO", "GONG"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["coronal-hole", "ensemble-segmentation", "magnetic-constraint", "filament", "chromosphere", "hmi"]
...
links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2405.04731"
  ads_url: null
  code_repo: null
  data_repo: null
claim_boundary:
  scope: >-
    Stub: A magnetic-constrained ensemble reduces filament false positives and improves CH boundary stability vs single-method baselines. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
- Running an ensemble CH-segmentation pipeline constrained by HMI / GONG photospheric magnetic-field data.
...
**Claim (narrow form).** A magnetic-constrained ensemble reduces filament false positives and improves CH boundary stability vs single-method baselines. The narrow form below should be tightened once the full text is verified:
...
- arXiv: https://arxiv.org/abs/2405.04731
...
Stub built from theme_solar_wind_segmentation.json entry arxiv_id=2405.04731. Author list, ensemble member set and metric TODO verify.
