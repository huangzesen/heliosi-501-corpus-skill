---
name: paper-fung-2019-clustering-helio-streams
description: >-
  Use when applying K-Means / GMM to multi-feature solar-wind vectors and tracking cluster occurrence across cycles 23–25 — central paper claim is
  K-Means / GMM clustering reproduces canonical regimes and yields cycle-modulated occurrence statistics (TODO verify 2019).
version: 0.1.0
kind: paper-skill
quality: stub
...
paper:
  title: "K-Means / GMM clustering of solar-wind streams across solar cycles"
  first_author: "TODO verify (Fung / clustering lineage)"
...
domain:
  ...
  secondary_themes: ["machine-learning", "unsupervised-classification", "kmeans", "gaussian-mixture"]
  missions: ["ACE", "Wind"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["solar-wind", "classification", "kmeans", "gmm", "clustering", "solar-cycle"]
...
claim_boundary:
  scope: >-
    Stub: K-Means / GMM clustering reproduces canonical regimes and yields cycle-modulated occurrence statistics. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
depends_on: ["paper-regan-2026-mars-solar-wind-ml-classification"]
adapter_notes: []
research_generation_affordances:
  - type: gap
    statement: "Stub skill — methods, data contracts, and validation target not yet compiled from the primary source."
    related_skills: ["paper-regan-2026-mars-solar-wind-ml-classification"]
...
- Applying K-Means / GMM to multi-feature solar-wind vectors and tracking cluster occurrence across cycles 23–25.
...
**Claim (narrow form).** K-Means / GMM clustering reproduces canonical regimes and yields cycle-modulated occurrence statistics. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: K-Means / GMM clustering reproduces canonical regimes and yields cycle-modulated occurrence statistics. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
- `[[paper-regan-2026-mars-solar-wind-ml-classification]]` — sibling slug; relation TODO verify in full text.

**Research-generation affordances.** Forward-pointing surface of the skill (spec §4 Layer 4):

- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-regan-2026-mars-solar-wind-ml-classification]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Stub anchor; primary author / venue / arXiv TODO verify.
