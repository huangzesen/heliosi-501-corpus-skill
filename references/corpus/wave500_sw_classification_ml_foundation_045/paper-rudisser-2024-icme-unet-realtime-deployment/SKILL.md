...
description: >-
  Use when deploying a U-Net ICME detector against near-real-time telemetry with operational latency budgets — central paper claim is
  A real-time U-Net ICME detector preserves Rüdisser-2022 TSS while running within minutes of telemetry arrival (TODO verify (likely Space Weather) 2024).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["event-detection", "icme", "unet", "real-time"]
  missions: ["Wind", "STEREO", "SolO"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["icme", "unet", "event-detection", "real-time", "wind", "stereo", "solar-orbiter"]
...
claim_boundary:
  scope: >-
    Stub: A real-time U-Net ICME detector preserves Rüdisser-2022 TSS while running within minutes of telemetry arrival. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# Real-time deployment of a U-Net ICME detector on Wind / STEREO / Solar Orbiter t — paper-skill (stub)

> Compiled from H. T. Rüdisser (successor lineage) (2024), "Real-time deployment of a U-Net ICME detector on Wind / STEREO / Solar Orbiter telemetry", TODO verify (likely Space Weather), TODO verify arXiv ID.
...
- Deploying a U-Net ICME detector against near-real-time telemetry with operational latency budgets.
...
**Claim (narrow form).** A real-time U-Net ICME detector preserves Rüdisser-2022 TSS while running within minutes of telemetry arrival. The narrow form below should be tightened once the full text is verified:
...
Hypothetical follow-up; bibliographic anchor TODO verify before promotion past stub.
