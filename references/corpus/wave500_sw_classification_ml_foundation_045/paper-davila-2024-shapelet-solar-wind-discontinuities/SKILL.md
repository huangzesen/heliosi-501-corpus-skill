---
name: paper-davila-2024-shapelet-solar-wind-discontinuities
description: >-
  Use when training shapelet classifiers to discriminate discontinuity types in solar-wind B-field time series — central paper claim is
  Shapelet classifiers separate rotational vs tangential discontinuities at TSS ≈ TODO verify on 1-au data (TODO verify 2024).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["time-series", "shapelet", "discontinuities", "classical-ml"]
  missions: ["Wind", "ACE"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["shapelet", "time-series", "discontinuities", "solar-wind", "classification"]
...
claim_boundary:
  scope: >-
    Stub: Shapelet classifiers separate rotational vs tangential discontinuities at TSS ≈ TODO verify on 1-au data. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
- Training shapelet classifiers to discriminate discontinuity types in solar-wind B-field time series.
...
**Claim (narrow form).** Shapelet classifiers separate rotational vs tangential discontinuities at TSS ≈ TODO verify on 1-au data. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: Shapelet classifiers separate rotational vs tangential discontinuities at TSS ≈ TODO verify on 1-au data. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
