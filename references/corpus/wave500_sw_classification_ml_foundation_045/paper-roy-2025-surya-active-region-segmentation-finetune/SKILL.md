---
name: paper-roy-2025-surya-active-region-segmentation-finetune
description: >-
  Use when fine-tuning Surya for active-region segmentation on SDO AIA/HMI imagery — central paper claim is
  LoRA fine-tuning of Surya recovers active-region segmentation masks competitive with task-specific baselines (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["foundation-model", "active-region-segmentation", "lora", "downstream-task"]
  missions: ["SDO"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["surya", "foundation-model", "active-region", "segmentation", "lora", "sdo"]
...
claim_boundary:
  scope: >-
    Stub: LoRA fine-tuning of Surya recovers active-region segmentation masks competitive with task-specific baselines. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
# Surya foundation-model fine-tune for active-region segmentation on SDO AIA/HMI — paper-skill (stub)

> Compiled from S. Roy (2025), "Surya foundation-model fine-tune for active-region segmentation on SDO AIA/HMI", TODO verify, arXiv:2508.14112.
...
- Fine-tuning Surya for active-region segmentation on SDO AIA/HMI imagery.
...
**Claim (narrow form).** LoRA fine-tuning of Surya recovers active-region segmentation masks competitive with task-specific baselines. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: LoRA fine-tuning of Surya recovers active-region segmentation masks competitive with task-specific baselines. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
Companion downstream-task slug; segmentation metric (IoU / Dice) and AR catalog ground truth TODO verify.
