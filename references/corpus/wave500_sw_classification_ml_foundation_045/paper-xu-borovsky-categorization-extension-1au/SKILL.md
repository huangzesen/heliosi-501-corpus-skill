...
description: >-
  Use when applying or refining the Xu–Borovsky physical 4-class (coronal-hole / streamer-belt / sector-reversal / ejecta) labelling at 1 au — central paper claim is
  Re-derives the 4-class label boundaries used by downstream supervised classifiers (ε ≈ class-balanced accuracy TODO verify) (TODO verify (likely J. Geophys. Res. Space Phys.) 2020).
version: 0.1.0
kind: paper-skill
quality: stub
...
trigger_keywords: ["xu-borovsky", "solar-wind", "labelling", "categorization", "1au", "supervised"]
...
- Applying or refining the Xu–Borovsky physical 4-class (coronal-hole / streamer-belt / sector-reversal / ejecta) labelling at 1 au.
...
**Claim (narrow form).** Re-derives the 4-class label boundaries used by downstream supervised classifiers (ε ≈ class-balanced accuracy TODO verify). The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: Re-derives the 4-class label boundaries used by downstream supervised classifiers (ε ≈ class-balanced accuracy TODO verify). The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
Stub: the original Xu & Borovsky 2015 paper is not yet in the corpus; this slug captures the operational labelling-extension lineage. Citation TODO verify against the original article.
