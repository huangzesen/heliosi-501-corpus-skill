---
name: paper-illarionov-2020-cnn-coronal-hole-segmentation
description: >-
  Use when training a CNN for pixel-wise coronal-hole segmentation in EUV channels — central paper claim is
  A U-Net-style CNN segments coronal holes from EUV imagery with X% IoU on a held-out validation set (TODO verify) (TODO verify 2020).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["image-segmentation", "coronal-hole", "cnn", "deep-learning"]
  missions: ["SDO", "STEREO"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["coronal-hole", "cnn", "unet", "segmentation", "euv", "deep-learning"]
...
links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2207.10070"
  ads_url: null
  code_repo: null
  data_repo: null
claim_boundary:
  scope: >-
    Stub: A U-Net-style CNN segments coronal holes from EUV imagery with X% IoU on a held-out validation set (TODO verify). The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
- Training a CNN for pixel-wise coronal-hole segmentation in EUV channels.
...
**Claim (narrow form).** A U-Net-style CNN segments coronal holes from EUV imagery with X% IoU on a held-out validation set (TODO verify). The narrow form below should be tightened once the full text is verified:
...
- arXiv: https://arxiv.org/abs/2207.10070
...
Stub built from theme_solar_wind_segmentation.json entry arxiv_id=2207.10070. Author list, training-set details and IoU TODO verify.
