---
name: paper-roy-2025-surya-finetune-solar-wind-speed-forecast
description: >-
  Use when fine-tuning the Surya heliophysics foundation model on a solar-wind speed forecasting target at L1 — central paper claim is
  LoRA fine-tuning of the 366M-parameter Surya model on SDO inputs produces competitive solar-wind speed forecasts at L1 (TODO verify (Surya downstream task report) 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["foundation-model", "transformer", "solar-wind-forecast", "lora", "downstream-task"]
  missions: ["SDO", "ACE", "DSCOVR"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["surya", "foundation-model", "solar-wind", "forecast", "lora", "fine-tune", "transformer"]
...
links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2508.14112"
  ads_url: null
  code_repo: null
  data_repo: null
claim_boundary:
  scope: >-
    Stub: LoRA fine-tuning of the 366M-parameter Surya model on SDO inputs produces competitive solar-wind speed forecasts at L1. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
depends_on: ["paper-roy-2025-surya-heliophysics-foundation-model", "paper-roy-2025-suryabench-ml-benchmark-dataset"]
adapter_notes: []
research_generation_affordances:
  - type: gap
    statement: "Stub skill — methods, data contracts, and validation target not yet compiled from the primary source."
    related_skills: ["paper-roy-2025-surya-heliophysics-foundation-model", "paper-roy-2025-suryabench-ml-benchmark-dataset"]
...
# Surya foundation-model fine-tune for solar-wind speed forecasting at L1 — paper-skill (stub)

> Compiled from S. Roy (2025), "Surya foundation-model fine-tune for solar-wind speed forecasting at L1", TODO verify (Surya downstream task report), arXiv:2508.14112.
...
- Fine-tuning the Surya heliophysics foundation model on a solar-wind speed forecasting target at L1.
...
**Claim (narrow form).** LoRA fine-tuning of the 366M-parameter Surya model on SDO inputs produces competitive solar-wind speed forecasts at L1. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: LoRA fine-tuning of the 366M-parameter Surya model on SDO inputs produces competitive solar-wind speed forecasts at L1. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
- `[[paper-roy-2025-surya-heliophysics-foundation-model]]` — sibling slug; relation TODO verify in full text.
- `[[paper-roy-2025-suryabench-ml-benchmark-dataset]]` — sibling slug; relation TODO verify in full text.

**Research-generation affordances.** Forward-pointing surface of the skill (spec §4 Layer 4):

- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-roy-2025-surya-heliophysics-foundation-model]]`, `[[paper-roy-2025-suryabench-ml-benchmark-dataset]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Captures the solar-wind-speed downstream task referenced in the Surya abstract; numerical fine-tune accuracy / window length / loss spec TODO verify in the full paper or companion technical report.
