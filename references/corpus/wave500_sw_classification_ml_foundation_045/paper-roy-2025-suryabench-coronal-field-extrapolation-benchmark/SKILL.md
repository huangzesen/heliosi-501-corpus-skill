---
name: paper-roy-2025-suryabench-coronal-field-extrapolation-benchmark
description: >-
  Use when running the SuryaBench coronal-field-extrapolation sub-task as an ML benchmark — central paper claim is
  SuryaBench publishes a labelled coronal-field-extrapolation benchmark anchored on HMI magnetograms (TODO verify 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes: ["benchmark", "dataset", "coronal-field-extrapolation", "pfss"]
  missions: ["SDO"]
  regime: []  # TODO verify (sub-Alfvenic | super-Alfvenic | inner-heliosphere | 1au | corona | ...)
trigger_keywords: ["suryabench", "coronal-field-extrapolation", "pfss", "benchmark", "dataset"]
...
links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2508.14107"
  ads_url: null
  code_repo: null
  data_repo: null
claim_boundary:
  scope: >-
    Stub: SuryaBench publishes a labelled coronal-field-extrapolation benchmark anchored on HMI magnetograms. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
  out_of_scope:
...
depends_on: ["paper-roy-2025-suryabench-ml-benchmark-dataset", "paper-stansby-2020-pfsspy-python-pfss"]
adapter_notes: []
research_generation_affordances:
  - type: gap
    statement: "Stub skill — methods, data contracts, and validation target not yet compiled from the primary source."
    related_skills: ["paper-roy-2025-suryabench-ml-benchmark-dataset", "paper-stansby-2020-pfsspy-python-pfss"]
...
# SuryaBench — coronal-field extrapolation benchmark task — paper-skill (stub)

> Compiled from S. Roy (2025), "SuryaBench — coronal-field extrapolation benchmark task", TODO verify, arXiv:2508.14107.
...
- Running the SuryaBench coronal-field-extrapolation sub-task as an ML benchmark.
...
**Claim (narrow form).** SuryaBench publishes a labelled coronal-field-extrapolation benchmark anchored on HMI magnetograms. The narrow form below should be tightened once the full text is verified:
...
**In scope.** Stub: SuryaBench publishes a labelled coronal-field-extrapolation benchmark anchored on HMI magnetograms. The narrow scope (mission, distance range, wind type, time-window) is TODO verify against the primary source. At stub tier this skill is referenced by slug and listed in the wave-level index; do not inherit the headline numbers as benchmarked claims.
...
- `[[paper-stansby-2020-pfsspy-python-pfss]]` — sibling slug; relation TODO verify in full text.

**Research-generation affordances.** Forward-pointing surface of the skill (spec §4 Layer 4):

- **Gap** — Stub skill; the executable protocol (Layer 2) is not yet compiled. Related: `[[paper-roy-2025-suryabench-ml-benchmark-dataset]]`, `[[paper-stansby-2020-pfsspy-python-pfss]]`. Proposed: promote to method-ready by populating §3/§4/§5 from the primary source.
...
Sub-benchmark slug; ground-truth field model and error metric TODO verify.
