---
name: paper-zheng-2025-ip-shock-effect-turbulence
description: >-
  Use when ip shock turbulence or related diagnostics surface in a SEP / shock / space-weather workflow — central claim: IP shocks modify the upstream/downstream turbulence parameters (spectral index, anisotropy, compressive fraction), and the modification correlates with shock parameters. (arXiv:2505.04450, 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes:
    ...
    - shocks
    - space-weather
  missions: [Wind, ACE, STEREO]
  regime: [inner-heliosphere]

trigger_keywords:
  - "IP shock turbulence"
  - "upstream / downstream turbulence"
  - "compressive ratio"
  - "anisotropy change"

data_products:
  - instrument: "Wind/MFI + 3DP + SWE"
    level: "L2"
    cadence: "high cadence"
    interval: "IP-shock catalog window"
    archive: "NASA CDAWeb"
  - instrument: "ACE/MAG + SWEPAM"
    level: "L2"
    cadence: "high cadence"
    interval: "IP-shock catalog window"
    archive: "NASA CDAWeb"
...
algorithms:
  - name: "Per-shock upstream / downstream window definition"
    equation_refs: []
    external_implementations: []
  - name: "Spectral index + anisotropy fit"
    equation_refs: []
    external_implementations: []
  - name: "Cross-event regression against shock-Mach / θ_Bn"
    equation_refs: []
    external_implementations: []
...
claim_boundary:
  scope: >-
    IP-shock catalog (TODO_verify); upstream/downstream turbulence analysis.
  out_of_scope:
    - "Do not generalize to coronal shocks without separate analysis."

failure_modes:
  - "Window-length sensitivity"
  - "Local plasma-frame frame definition"
...
research_generation_affordances:
  - type: "gap"
    statement: "No PSP near-Sun shock turbulence-modification analogue."
    related_skills: []
  - type: "minimal_experiment"
    statement: "Repeat analysis on PSP-near-Sun IP shocks; compare modification factors."
    related_skills: []

provenance:
  generated_by: "HelioSI paper-to-skill factory (Claude Opus 4.7) — wave500 SEP/shocks/space-weather batch"
  generated_at: "2026-05-18T00:00:00Z"
  source_record: "sioulas-reproduction/results/arxiv_papers/theme_energetic_particles.json#arxiv:2505.04450"
  verified_by: null
  verified_at: null
...
# Effect of interplanetary shock waves on turbulence parameters — paper-skill

> Compiled from arXiv:2505.04450 (2025), unverified author et al.
...
**Claim (narrow form).** IP shocks modify the upstream/downstream turbulence parameters (spectral index, anisotropy, compressive fraction), and the modification correlates with shock parameters.

**Verifiable task.** A reproduction succeeds when an agent reads the
abstract capability contract in §3–§4 and reproduces Per-class turbulence-parameter shifts (TODO_verify numerics). within
...
### Per-shock upstream / downstream window definition

- Procedure (abstract, runtime-neutral):
  1. TODO_verify_with_full_text — paraphrased from inventory abstract only.
- Capability requirements:
  - Per-shock upstream / downstream window definition as a callable on the data products in §4.

### Spectral index + anisotropy fit

- Procedure (abstract, runtime-neutral):
  1. TODO_verify_with_full_text — paraphrased from inventory abstract only.
- Capability requirements:
  - Spectral index + anisotropy fit as a callable on the data products in §4.

### Cross-event regression against shock-Mach / θ_Bn

- Procedure (abstract, runtime-neutral):
  1. TODO_verify_with_full_text — paraphrased from inventory abstract only.
- Capability requirements:
  - Cross-event regression against shock-Mach / θ_Bn as a callable on the data products in §4.
...
|------------|-------|---------|----------|---------|------------------------|
| Wind/MFI + 3DP + SWE | L2 | high cadence | IP-shock catalog window | NASA CDAWeb | abstract: load + decode + subset |
| ACE/MAG + SWEPAM | L2 | high cadence | IP-shock catalog window | NASA CDAWeb | abstract: load + decode + subset |
...
reference. The target is: Per-class turbulence-parameter shifts (TODO_verify numerics).
...
**In scope.** IP-shock catalog (TODO_verify); upstream/downstream turbulence analysis.
...
- `[[paper-trotta-2025-ip-shock-variability-multi-spacecraft]]` — assumed for context (see linked skill).

**Research-generation affordances** (also in
`research_generation_affordances[]` so a graph walker can ingest them
without re-parsing prose):

- **Gap** — No PSP near-Sun shock turbulence-modification analogue.
- **Minimal_experiment** — Repeat analysis on PSP-near-Sun IP shocks; compare modification factors.
