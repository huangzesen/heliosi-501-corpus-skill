---
name: paper-jebaraj-2024-type-ii-multi-vantage-catalog
description: >-
  Use when type ii catalog or related diagnostics surface in a SEP / shock / space-weather workflow — central claim: A multi-vantage-point catalog of type II radio bursts (Wind/STEREO/PSP/SOLO) provides population-level statistics and correlations between type II properties and source-event parameters. (arXiv:2410.00814, 2024).
version: 0.1.0
kind: paper-skill
quality: stub
...
domain:
  ...
  secondary_themes:
    ...
    - shocks
    - space-weather
  missions: [Wind, STEREO, PSP, Solar Orbiter]
  regime: [inner-heliosphere]

trigger_keywords:
  - "type II catalog"
  - "multi-vantage point"
  - "Wind STEREO PSP SOLO radio"

data_products:
  - instrument: "Wind/WAVES + STEREO/WAVES + PSP/FIELDS RFS + Solar Orbiter RPW"
    level: "L2"
    cadence: "minute"
    interval: "Catalog window (TODO_verify)"
    archive: "agency archives"

algorithms:
  - name: "Cross-mission type II event matching"
    equation_refs: []
    external_implementations: []
  - name: "Catalog assembly + per-event property table"
    equation_refs: []
    external_implementations: []
  - name: "Correlation analysis vs CME/flare/SEP variables"
    equation_refs: []
    external_implementations: []
...
links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2410.00814"
  ads_url: null
  code_repo: null
  data_repo: null
...
claim_boundary:
  scope: >-
    Catalog paper; multi-vantage-point coverage; statistical correlations only (no event-specific modeling).
  out_of_scope:
    - "Do not use as event-specific shock-strength estimator."

failure_modes:
  - "Cross-mission radio-band calibration"
  - "Selection bias from mission visibility periods"
...
research_generation_affordances:
  - type: "gap"
    statement: "Catalog could be extended with VLBI/LOFAR localizations."
    related_skills: ["paper-buthelezi-2025-resolving-shock-loft-type-ii-lofar"]
  - type: "minimal_experiment"
    statement: "Cross-match the catalog with the PSP IVA event list; test type II ↔ IVA correlation."
    related_skills: []

provenance:
  generated_by: "HelioSI paper-to-skill factory (Claude Opus 4.7) — wave500 SEP/shocks/space-weather batch"
  generated_at: "2026-05-18T00:00:00Z"
  source_record: "sioulas-reproduction/results/arxiv_papers/theme_energetic_particles.json#arxiv:2410.00814"
  verified_by: null
  verified_at: null
...
**Claim (narrow form).** A multi-vantage-point catalog of type II radio bursts (Wind/STEREO/PSP/SOLO) provides population-level statistics and correlations between type II properties and source-event parameters.

**Verifiable task.** A reproduction succeeds when an agent reads the
abstract capability contract in §3–§4 and reproduces Catalog entry count; correlation coefficients vs event variables (TODO_verify numerics). within
...
### Cross-mission type II event matching

- Procedure (abstract, runtime-neutral):
  1. TODO_verify_with_full_text — paraphrased from inventory abstract only.
- Capability requirements:
  - Cross-mission type II event matching as a callable on the data products in §4.

### Catalog assembly + per-event property table

- Procedure (abstract, runtime-neutral):
  1. TODO_verify_with_full_text — paraphrased from inventory abstract only.
- Capability requirements:
  - Catalog assembly + per-event property table as a callable on the data products in §4.

### Correlation analysis vs CME/flare/SEP variables

- Procedure (abstract, runtime-neutral):
  1. TODO_verify_with_full_text — paraphrased from inventory abstract only.
- Capability requirements:
  - Correlation analysis vs CME/flare/SEP variables as a callable on the data products in §4.
...
|------------|-------|---------|----------|---------|------------------------|
| Wind/WAVES + STEREO/WAVES + PSP/FIELDS RFS + Solar Orbiter RPW | L2 | minute | Catalog window (TODO_verify) | agency archives | abstract: load + decode + subset |
...
reference. The target is: Catalog entry count; correlation coefficients vs event variables (TODO_verify numerics).
...
**In scope.** Catalog paper; multi-vantage-point coverage; statistical correlations only (no event-specific modeling).
...
- arXiv: https://arxiv.org/abs/2410.00814
...
- **Gap** — Catalog could be extended with VLBI/LOFAR localizations. Related: `[[paper-buthelezi-2025-resolving-shock-loft-type-ii-lofar]]`.
- **Minimal_experiment** — Cross-match the catalog with the PSP IVA event list; test type II ↔ IVA correlation.
