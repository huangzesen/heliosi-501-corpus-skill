---
name: paper-ding-2025-2022-09-05-time-dependent-dsa
description: >-
  Use when 2022-09-05 labor day event or related diagnostics surface in a SEP / shock / space-weather workflow — central claim: The 2022-09-05 Labor Day SEP event shows direct evidence for time-dependent diffusive shock acceleration in the PSP near-Sun observations, with progressive spectral hardening matching the model. (arXiv:2506.20322, 2025).
version: 0.1.0
kind: paper-skill
quality: stub
...
algorithms:
  - name: "Per-energy onset / peak identification"
    equation_refs: []
    external_implementations: []
  - name: "Spectral-index time series"
    equation_refs: []
    external_implementations: []
  - name: "Time-dependent DSA fit"
    equation_refs: []
    external_implementations: []
...
links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2506.20322"
  ads_url: null
  code_repo: null
  data_repo: null
...
claim_boundary:
  scope: >-
    Single event 2022-09-05; PSP near-perihelion + 1 au observers; time-dependent DSA fit.
  out_of_scope:
    - "Do not generalize from one canonical event."

failure_modes:
  - "Background subtraction at low energies"
  - "Time-dependent DSA model identifiability"
...
research_generation_affordances:
  - type: "gap"
    statement: "Cross-event time-dependent DSA test missing across the PSP IVA list."
    related_skills: []
  - type: "minimal_experiment"
    statement: "Apply the same fit to 3 additional PSP-era IVA events; compare temporal hardening rates."
    related_skills: []

provenance:
  generated_by: "HelioSI paper-to-skill factory (Claude Opus 4.7) — wave500 SEP/shocks/space-weather batch"
  generated_at: "2026-05-18T00:00:00Z"
  source_record: "sioulas-reproduction/results/arxiv_papers/theme_energetic_particles.json#arxiv:2506.20322"
  verified_by: null
  verified_at: null
...
**Claim (narrow form).** The 2022-09-05 Labor Day SEP event shows direct evidence for time-dependent diffusive shock acceleration in the PSP near-Sun observations, with progressive spectral hardening matching the model.

**Verifiable task.** A reproduction succeeds when an agent reads the
abstract capability contract in §3–§4 and reproduces Spectral-index time evolution consistent with time-dependent DSA (TODO_verify numerics). within
...
### Per-energy onset / peak identification

- Procedure (abstract, runtime-neutral):
  1. TODO_verify_with_full_text — paraphrased from inventory abstract only.
- Capability requirements:
  - Per-energy onset / peak identification as a callable on the data products in §4.
...
- Procedure (abstract, runtime-neutral):
  1. TODO_verify_with_full_text — paraphrased from inventory abstract only.
- Capability requirements:
  - Spectral-index time series as a callable on the data products in §4.

### Time-dependent DSA fit

- Procedure (abstract, runtime-neutral):
  1. TODO_verify_with_full_text — paraphrased from inventory abstract only.
- Capability requirements:
  - Time-dependent DSA fit as a callable on the data products in §4.
...
| Multi-mission SEP context | L2 | instrument-native | 2022-09-05 ± 2 d | agency archives | abstract: load + decode + subset |
...
reference. The target is: Spectral-index time evolution consistent with time-dependent DSA (TODO_verify numerics).
...
**In scope.** Single event 2022-09-05; PSP near-perihelion + 1 au observers; time-dependent DSA fit.
...
- arXiv: https://arxiv.org/abs/2506.20322
...
- **Gap** — Cross-event time-dependent DSA test missing across the PSP IVA list.
- **Minimal_experiment** — Apply the same fit to 3 additional PSP-era IVA events; compare temporal hardening rates.
