---
name: paper-niemiec-2025-numerical-superdiffusive-particle-acceleration
description: >-
  Use when superdiffusive shock acceleration or related diagnostics surface in a SEP / shock / space-weather workflow — central claim: Numerical simulations of particle acceleration at IP shocks compare standard diffusive DSA with superdiffusive (anomalous) transport scenarios and identify observable signatures distinguishing them. (arXiv:2604.14819, 2026).
version: 0.1.0
kind: paper-skill
quality: stub
...
trigger_keywords:
  ...
  - "fractional-diffusion DSA"

data_products: []  # theory / modeling-only

algorithms:
  - name: "Standard DSA Fokker-Planck solver"
    equation_refs: []
    external_implementations: []
  - name: "Superdiffusive / fractional transport solver"
    equation_refs: []
    external_implementations: []
  - name: "Diagnostics that distinguish the two regimes"
    equation_refs: []
    external_implementations: []
...
links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2604.14819"
  ads_url: null
  code_repo: null
  data_repo: null
...
claim_boundary:
  scope: >-
    Simulation study (TODO_verify code); IP-shock parameter sweep; diffusive vs superdiffusive comparison.
  out_of_scope:
    - "Do not use as evidence for superdiffusive transport in real data without an observational test."

failure_modes:
  - "Numerical resolution / boundary conditions affect spectra"
  - "Superdiffusive parameter inversion is non-unique"
...
research_generation_affordances:
  - type: "gap"
    statement: "Observational tests of superdiffusive vs diffusive scenarios on PSP-era IP shocks are missing."
    related_skills: []
  - type: "minimal_experiment"
    statement: "Apply the simulated diagnostics to the 2022-09-05 + 2023-03-13 events and compare regimes."
    related_skills: []

provenance:
  generated_by: "HelioSI paper-to-skill factory (Claude Opus 4.7) — wave500 SEP/shocks/space-weather batch"
  generated_at: "2026-05-18T00:00:00Z"
  source_record: "sioulas-reproduction/results/arxiv_papers/theme_energetic_particles.json#arxiv:2604.14819"
  verified_by: null
  verified_at: null
...
**Claim (narrow form).** Numerical simulations of particle acceleration at IP shocks compare standard diffusive DSA with superdiffusive (anomalous) transport scenarios and identify observable signatures distinguishing them.

**Verifiable task.** A reproduction succeeds when an agent reads the
abstract capability contract in §3–§4 and reproduces Numerical-experiment intensity / spectrum signatures distinguishing scenarios (TODO_verify numerics). within
...
### Standard DSA Fokker-Planck solver

- Procedure (abstract, runtime-neutral):
  1. TODO_verify_with_full_text — paraphrased from inventory abstract only.
- Capability requirements:
  - Standard DSA Fokker-Planck solver as a callable on the data products in §4.

### Superdiffusive / fractional transport solver

- Procedure (abstract, runtime-neutral):
  1. TODO_verify_with_full_text — paraphrased from inventory abstract only.
- Capability requirements:
  - Superdiffusive / fractional transport solver as a callable on the data products in §4.

### Diagnostics that distinguish the two regimes

- Procedure (abstract, runtime-neutral):
  1. TODO_verify_with_full_text — paraphrased from inventory abstract only.
- Capability requirements:
  - Diagnostics that distinguish the two regimes as a callable on the data products in §4.
...
reference. The target is: Numerical-experiment intensity / spectrum signatures distinguishing scenarios (TODO_verify numerics).
...
**In scope.** Simulation study (TODO_verify code); IP-shock parameter sweep; diffusive vs superdiffusive comparison.
...
- arXiv: https://arxiv.org/abs/2604.14819
...
- **Gap** — Observational tests of superdiffusive vs diffusive scenarios on PSP-era IP shocks are missing.
- **Minimal_experiment** — Apply the simulated diagnostics to the 2022-09-05 + 2023-03-13 events and compare regimes.
