...
paper:
  ...
  year: 2022
  venue: "arXiv preprint / ICLR 2023"
  doi: null
  arxiv_id: "2210.03629"
  ads_bibcode: null
...
domain:
  primary_theme: other
  secondary_themes: ["agent-runtime", "agent-runtime-reason-act-loop"]
  missions: []
  regime: []

trigger_keywords:
  - "foundational-method"
  - "agent-runtime-reason-act-loop"
  - "agent-runtime"
  - "ai-scientist"
  - "positioning"
...
data_products: []
algorithms:
  - name: "Interleaved thought-action-observation trajectories"
  - name: "External-tool feedback grounding"
  - name: "Benchmark evaluation across QA + decision-making tasks"
...
claim_boundary:
  scope: >-
    Bounded to the four benchmarks reported; the synergy claim is empirical, not theoretical.
  out_of_scope:
  - "Do not claim ReAct generalizes monotonically — long horizons can still degrade."
  - "Do not treat the reasoning trace as faithful to the underlying decision."

failure_modes:
  - "Trace fluency does not imply trace faithfulness."
  - "Long-horizon drift; needs orthogonal memory or planning aid."
...
research_generation_affordances:
  - type: hypothesis
    statement: "Every HelioSI Layer-2 abstract procedure is a candidate for ReAct-style execution: thought (which paper-skill to invoke) -> action (call MCP) -> observation (data/figure) -> next thought."
  - type: minimal_experiment
    statement: "Re-run one HelioSI workflow (PFSS source mapping) with explicit ReAct traces logged; measure whether trace inspection helps the HelioSI PI catch reasoning errors."

provenance:
  generated_by: "HelioSI paper-to-skill factory (Claude Opus 4.7) - wave500_agent_runtime_eval_design_045"
  generated_at: "2026-05-18T00:00:00Z"
  source_record: "sioulas-reproduction/results/paper_skill_corpus/wave500_agent_runtime_eval_design_045/manifest.json and sioulas-reproduction/results/agent_runtime_paper_scan_raw.md / agent_runtime_2026_only_synthesis.md (arxiv:2210.03629, doi:n/a)"
  verified_by: null
  verified_at: null
...
# ReAct: Synergizing Reasoning and Acting in Language Models - paper-skill (wave500)
...
- designing a HelioSI benchmark or evaluation that should reflect the agent-runtime-reason-act-loop pattern
...
**Claim (narrow form).** Interleaving chain-of-thought reasoning and action steps (ReAct) outperforms pure reasoning or pure acting on HotpotQA, FEVER, and ALFWorld decision-making benchmarks, by letting the agent ground its plan in observed external feedback.
...
### Method 1: Interleaved thought-action-observation trajectories
...
### Method 2: External-tool feedback grounding

- Paper role: explicit named method/component of the paper's contribution.
- Abstract procedure: the runtime that wants to borrow this pattern must be able to (a) instantiate the component as a callable, (b) feed it the manuscript/benchmark/sibling-skill inputs from §4, and (c) emit an artifact a Layer-4 affordance can reference.

### Method 3: Benchmark evaluation across QA + decision-making tasks
...
**In scope.** Bounded to the four benchmarks reported; the synergy claim is empirical, not theoretical.
...
- **Hypothesis** - Every HelioSI Layer-2 abstract procedure is a candidate for ReAct-style execution: thought (which paper-skill to invoke) -> action (call MCP) -> observation (data/figure) -> next thought. Related: n/a.
- **Minimal_experiment** - Re-run one HelioSI workflow (PFSS source mapping) with explicit ReAct traces logged; measure whether trace inspection helps the HelioSI PI catch reasoning errors. Related: n/a.
