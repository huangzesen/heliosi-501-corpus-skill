...
name: paper-liweiwu-2026-ai-co-scientist-ranking-search-models
description: >-
  Use when the agent is positioning the consuming agent/manuscript against domain-specific-ai-scientist literature, borrowing a agent-runtime-full-pipeline-cloud design pattern, or auditing a benchmark/manuscript claim related to it - central claim is "An AI Co-Scientist framework automates the full search-ranking research pipeline (idea -> code -> GPU training -> analysis) with single-LLM agents for routine t" (arXiv preprint 2026).
version: 0.1.0
kind: paper-skill
...
domain:
  primary_theme: other
  secondary_themes: ["agent-runtime", "agent-runtime-full-pipeline-cloud"]
  missions: []
  regime: []

trigger_keywords:
  - "domain-specific-ai-scientist"
  - "agent-runtime-full-pipeline-cloud"
  - "agent-runtime"
  - "ai-scientist"
  - "positioning"
...
data_products: []
algorithms:
  - name: "Single-LLM routine agents"
  - name: "Multi-LLM consensus agents for analysis + idea generation"
  - name: "Cloud GPU training job scheduling"
  - name: "Expert-in-the-loop pipeline orchestration"

validation_target: null

links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2603.22376"
  ads_url: null
  code_repo: null
  data_repo: null
...
claim_boundary:
  scope: >-
    Bounded to commercial-search-ranking research with the named cloud + LLM consensus stack; expert-in-the-loop is required for the headline result.
  out_of_scope:
  - "Do not assume the multi-LLM-consensus pattern transfers to all phases without ablation."
  - "Do not generalize 'first study in ranking community' beyond ranking research."

failure_modes:
  - "Multi-LLM consensus is expensive; ablations vs. best-of-N or judge-based selection needed."
  - "Cloud-coupling makes the pattern depend on provider availability."
...
provenance:
  ...
  source_record: "sioulas-reproduction/results/paper_skill_corpus/wave500_agent_runtime_eval_design_045/manifest.json and sioulas-reproduction/results/agent_runtime_paper_scan_raw.md / agent_runtime_2026_only_synthesis.md (arxiv:2603.22376, doi:n/a)"
  verified_by: null
  verified_at: null
...
# AI Co-Scientist for Ranking: Discovering Novel Search Ranking Models alongside LLM-based AI Agents with Cloud Computing Access - paper-skill (wave500)
...
- designing a HelioSI benchmark or evaluation that should reflect the agent-runtime-full-pipeline-cloud pattern
...
**Claim (narrow form).** An AI Co-Scientist framework automates the full search-ranking research pipeline (idea -> code -> GPU training -> analysis) with single-LLM agents for routine tasks and multi-LLM consensus agents (GPT 5.2, Gemini Pro 3, Claude Opus 4.5) for challenging phases such as results analysis and idea generation, with expert in the loop and cloud computing access.
...
### Method 1: Single-LLM routine agents
...
### Method 2: Multi-LLM consensus agents for analysis + idea generation

- Paper role: explicit named method/component of the paper's contribution.
- Abstract procedure: the runtime that wants to borrow this pattern must be able to (a) instantiate the component as a callable, (b) feed it the manuscript/benchmark/sibling-skill inputs from §4, and (c) emit an artifact a Layer-4 affordance can reference.

### Method 3: Cloud GPU training job scheduling

- Paper role: explicit named method/component of the paper's contribution.
- Abstract procedure: the runtime that wants to borrow this pattern must be able to (a) instantiate the component as a callable, (b) feed it the manuscript/benchmark/sibling-skill inputs from §4, and (c) emit an artifact a Layer-4 affordance can reference.

### Method 4: Expert-in-the-loop pipeline orchestration
...
**In scope.** Bounded to commercial-search-ranking research with the named cloud + LLM consensus stack; expert-in-the-loop is required for the headline result.
...
- DOI: n/a
- arXiv: [arXiv:2603.22376](https://arxiv.org/abs/2603.22376)
- ADS: n/a
- Code: n/a
- Data: n/a
