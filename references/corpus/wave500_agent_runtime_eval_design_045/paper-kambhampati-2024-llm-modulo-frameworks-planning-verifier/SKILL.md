...
domain:
  primary_theme: other
  secondary_themes: ["agent-runtime", "agent-runtime-llm-plus-external-verifier"]
  missions: []
  regime: []

trigger_keywords:
  - "critique"
  - "agent-runtime-llm-plus-external-verifier"
  - "agent-runtime"
  - "ai-scientist"
  - "positioning"
...
data_products: []
algorithms:
  - name: "LLM proposer + critic in a loop"
  - name: "External sound verifier (model checker / simulator)"
  - name: "Iterative repair until verifier accepts"

validation_target: null

links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2402.01817"
  ads_url: null
  code_repo: null
  data_repo: null
...
claim_boundary:
  scope: >-
    Bounded to planning tasks where soundness can be checked externally; the claim is about plan correctness, not about LLMs as conversation partners.
  out_of_scope:
  - "Do not generalize 'cannot plan' to all reasoning."
  - "Do not assume an external verifier always exists in the target domain."

failure_modes:
  - "Verifier coverage gaps mask incorrect plans."
  - "Iteration cost without convergence guarantee."
...
research_generation_affordances:
  - type: hypothesis
    statement: "HelioSI's paper-skill validation_targets ARE the sound external verifiers in the LLM-Modulo sense. The runtime should refuse plans that no validation_target accepts."
  - type: minimal_experiment
    statement: "Wire one HelioSI workflow (PFSS open flux) as LLM-Modulo: LLM proposes a (r_ss, magnetogram) configuration; PFSS solver verifies open flux; iterate until convergence."

provenance:
  generated_by: "HelioSI paper-to-skill factory (Claude Opus 4.7) - wave500_agent_runtime_eval_design_045"
  generated_at: "2026-05-18T00:00:00Z"
  source_record: "sioulas-reproduction/results/paper_skill_corpus/wave500_agent_runtime_eval_design_045/manifest.json and sioulas-reproduction/results/agent_runtime_paper_scan_raw.md / agent_runtime_2026_only_synthesis.md (arxiv:2402.01817, doi:n/a)"
  verified_by: null
  verified_at: null
...
# LLMs Can't Plan, But Can Help Planning in LLM-Modulo Frameworks - paper-skill (wave500)
...
- designing a HelioSI benchmark or evaluation that should reflect the agent-runtime-llm-plus-external-verifier pattern
...
**Claim (narrow form).** LLMs cannot generate executable plans on their own at acceptable correctness; they CAN help in 'LLM-Modulo' frameworks where the LLM proposes + critiques and external sound verifiers (model checkers, simulators) accept or reject.
...
### Method 1: LLM proposer + critic in a loop
...
### Method 2: External sound verifier (model checker / simulator)

- Paper role: explicit named method/component of the paper's contribution.
- Abstract procedure: the runtime that wants to borrow this pattern must be able to (a) instantiate the component as a callable, (b) feed it the manuscript/benchmark/sibling-skill inputs from §4, and (c) emit an artifact a Layer-4 affordance can reference.

### Method 3: Iterative repair until verifier accepts
...
**In scope.** Bounded to planning tasks where soundness can be checked externally; the claim is about plan correctness, not about LLMs as conversation partners.
...
- DOI: n/a
- arXiv: [arXiv:2402.01817](https://arxiv.org/abs/2402.01817)
- ADS: n/a
- Code: n/a
- Data: n/a
...
- **Hypothesis** - HelioSI's paper-skill validation_targets ARE the sound external verifiers in the LLM-Modulo sense. The runtime should refuse plans that no validation_target accepts. Related: n/a.
- **Minimal_experiment** - Wire one HelioSI workflow (PFSS open flux) as LLM-Modulo: LLM proposes a (r_ss, magnetogram) configuration; PFSS solver verifies open flux; iterate until convergence. Related: n/a.
