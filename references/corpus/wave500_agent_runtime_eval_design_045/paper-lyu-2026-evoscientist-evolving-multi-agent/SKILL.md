...
name: paper-lyu-2026-evoscientist-evolving-multi-agent
description: >-
  Use when the agent is positioning the consuming agent/manuscript against ai-scientist-system literature, borrowing a agent-runtime-persistent-memory-evolution design pattern, or auditing a benchmark/manuscript claim related to it - central claim is "EvoScientist replaces static AI-scientist pipelines with an evolving multi-agent system that improves research strategies through persistent memory and self-evo" (arXiv preprint 2026).
version: 0.1.0
kind: paper-skill
...
paper:
  ...
  first_author: "Lyu, Y."
  authors: ["Yougang Lyu", "Xi Zhang", "Xinhao Yi", "Yuyue Zhao", "Shuyu Guo", "Wenxiang Hu"]
  year: 2026
...
domain:
  primary_theme: other
  secondary_themes: ["agent-runtime", "agent-runtime-persistent-memory-evolution"]
  missions: []
  regime: []

trigger_keywords:
  - "ai-scientist-system"
  - "agent-runtime-persistent-memory-evolution"
  - "agent-runtime"
  - "ai-scientist"
  - "positioning"
...
data_products: []
algorithms:
  - name: "Persistent procedural memory store"
  - name: "Self-evolution: failed-experiment memory + successful-strategy reinforcement"
  - name: "Three-agent specialization (Researcher + experiment + reviewer roles)"
...
claim_boundary:
  scope: >-
    Claim is that persistent memory + self-evolution outperforms static pipelines on the paper's chosen benchmarks; bounded by those benchmarks.
  out_of_scope:
  - "Do not treat 'persistent memory' as automatically generalizing across domains without re-grounding the memory schema."
  - "Do not claim that evolution always converges — pathological loops are possible."

failure_modes:
  - "Memory schema must match the domain; reusing it cross-domain risks brittle generalization."
  - "Self-evolution can amplify spurious correlations in early successes."
...
research_generation_affordances:
  - type: hypothesis
    statement: "HelioSI's paper-skill corpus + LingTai persistent memory + molting is the heliophysics instantiation of EvoScientist's persistent-memory thesis. HelioSI should cite this as the direct positioning anchor."
  - type: minimal_experiment
    statement: "Track every failed CDAWeb fetch / failed PFSS solve in HelioSI's memory, and measure whether subsequent attempts at the same task succeed at higher rates over time — the empirical signature of self-evolution."

provenance:
  generated_by: "HelioSI paper-to-skill factory (Claude Opus 4.7) - wave500_agent_runtime_eval_design_045"
  generated_at: "2026-05-18T00:00:00Z"
  source_record: "sioulas-reproduction/results/paper_skill_corpus/wave500_agent_runtime_eval_design_045/manifest.json and sioulas-reproduction/results/agent_runtime_paper_scan_raw.md / agent_runtime_2026_only_synthesis.md (arxiv:2603.08127, doi:n/a)"
  verified_by: null
  verified_at: null
...
# EvoScientist: Towards Multi-Agent Evolving AI Scientists for End-to-End Scientific Discovery - paper-skill (wave500)
...
- designing a HelioSI benchmark or evaluation that should reflect the agent-runtime-persistent-memory-evolution pattern
...
**Claim (narrow form).** EvoScientist replaces static AI-scientist pipelines with an evolving multi-agent system that improves research strategies through persistent memory and self-evolution across accumulated interaction histories; three specialized agents (Researcher Agent + downstream roles) drive idea generation and execution.
...
### Method 1: Persistent procedural memory store
...
### Method 2: Self-evolution: failed-experiment memory + successful-strategy reinforcement

- Paper role: explicit named method/component of the paper's contribution.
- Abstract procedure: the runtime that wants to borrow this pattern must be able to (a) instantiate the component as a callable, (b) feed it the manuscript/benchmark/sibling-skill inputs from §4, and (c) emit an artifact a Layer-4 affordance can reference.

### Method 3: Three-agent specialization (Researcher + experiment + reviewer roles)
...
**In scope.** Claim is that persistent memory + self-evolution outperforms static pipelines on the paper's chosen benchmarks; bounded by those benchmarks.
...
- DOI: n/a
- arXiv: [arXiv:2603.08127](https://arxiv.org/abs/2603.08127)
- ADS: n/a
- Code: n/a
- Data: n/a
...
- **Hypothesis** - HelioSI's paper-skill corpus + LingTai persistent memory + molting is the heliophysics instantiation of EvoScientist's persistent-memory thesis. HelioSI should cite this as the direct positioning anchor. Related: n/a.
- **Minimal_experiment** - Track every failed CDAWeb fetch / failed PFSS solve in HelioSI's memory, and measure whether subsequent attempts at the same task succeed at higher rates over time — the empirical signature of self-evolution. Related: n/a.
