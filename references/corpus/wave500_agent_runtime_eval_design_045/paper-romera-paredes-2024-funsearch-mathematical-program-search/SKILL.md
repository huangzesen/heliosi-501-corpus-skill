...
domain:
  primary_theme: other
  secondary_themes: ["agent-runtime", "agent-runtime-program-search-discovery"]
  missions: []
  regime: []

trigger_keywords:
  - "domain-specific-ai-scientist"
  - "agent-runtime-program-search-discovery"
  - "agent-runtime"
  - "ai-scientist"
  - "positioning"
...
data_products: []
algorithms:
  - name: "LLM-as-program-proposer"
  - name: "Deterministic evaluator over fixed score function"
  - name: "Evolutionary population maintenance"

validation_target: null

links:
  doi_url: "https://doi.org/10.1038/s41586-023-06924-6"
  arxiv_url: null
  ads_url: null
  code_repo: null
  data_repo: null
...
claim_boundary:
  scope: >-
    Bounded to mathematical/combinatorial problems with deterministic evaluators; novelty is checked against published bests.
  out_of_scope:
  - "Do not generalize to problems lacking a fast, deterministic program evaluator."
  - "Do not assume LLM-proposal quality scales arbitrarily."

failure_modes:
  - "Score saturation if the search space is convex around current best."
  - "Evaluator faithfulness is critical; flawed scoring misleads search."
...
research_generation_affordances:
  - type: hypothesis
    statement: "FunSearch's pattern transplants to heliophysics: LLM proposes a Python algorithm (e.g. an improved structure-function estimator); evaluator runs against a benchmark interval (e.g. PSP E2) and scores against a published reference value. The HelioSI corpus's validation_targets are the scoring functions."
  - type: minimal_experiment
    statement: "Run FunSearch over candidate Politano-Pouquet epsilon estimators on a single PSP interval, evaluated against the bandyopadhyay-2020 reference cascade-rate value; report best estimator vs. published value."

provenance:
  generated_by: "HelioSI paper-to-skill factory (Claude Opus 4.7) - wave500_agent_runtime_eval_design_045"
  generated_at: "2026-05-18T00:00:00Z"
  source_record: "sioulas-reproduction/results/paper_skill_corpus/wave500_agent_runtime_eval_design_045/manifest.json and sioulas-reproduction/results/agent_runtime_paper_scan_raw.md / agent_runtime_2026_only_synthesis.md (arxiv:n/a, doi:10.1038/s41586-023-06924-6)"
  verified_by: null
  verified_at: null
...
# Mathematical discoveries from program search with large language models (FunSearch) - paper-skill (wave500)
...
- designing a HelioSI benchmark or evaluation that should reflect the agent-runtime-program-search-discovery pattern
...
**Claim (narrow form).** FunSearch pairs an LLM (proposes programs) with an evaluator (runs the program) in an evolutionary loop; discovers new mathematical constructions (improved cap-set lower bounds; new bin-packing heuristics) beyond best-known results.
...
### Method 1: LLM-as-program-proposer
...
### Method 2: Deterministic evaluator over fixed score function

- Paper role: explicit named method/component of the paper's contribution.
- Abstract procedure: the runtime that wants to borrow this pattern must be able to (a) instantiate the component as a callable, (b) feed it the manuscript/benchmark/sibling-skill inputs from §4, and (c) emit an artifact a Layer-4 affordance can reference.

### Method 3: Evolutionary population maintenance
...
**In scope.** Bounded to mathematical/combinatorial problems with deterministic evaluators; novelty is checked against published bests.
...
- **Hypothesis** - FunSearch's pattern transplants to heliophysics: LLM proposes a Python algorithm (e.g. an improved structure-function estimator); evaluator runs against a benchmark interval (e.g. PSP E2) and scores against a published reference value. The HelioSI corpus's validation_targets are the scoring functions. Related: n/a.
- **Minimal_experiment** - Run FunSearch over candidate Politano-Pouquet epsilon estimators on a single PSP interval, evaluated against the bandyopadhyay-2020 reference cascade-rate value; report best estimator vs. published value. Related: n/a.
