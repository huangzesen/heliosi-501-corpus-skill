...
name: paper-shinn-2023-reflexion-verbal-reinforcement-agents
description: >-
  Use when the agent is positioning the consuming agent/manuscript against foundational-method literature, borrowing a agent-runtime-self-reflection-loop design pattern, or auditing a benchmark/manuscript claim related to it - central claim is "Reflexion adds verbal self-reflection on past failures to an LLM agent loop, improving performance on coding (HumanEval), web navigation (AlfWorld), and reasoni" (arXiv preprint / NeurIPS 2023 2023).
version: 0.1.0
kind: paper-skill
...
domain:
  primary_theme: other
  secondary_themes: ["agent-runtime", "agent-runtime-self-reflection-loop"]
  missions: []
  regime: []

trigger_keywords:
  - "foundational-method"
  - "agent-runtime-self-reflection-loop"
  - "agent-runtime"
  - "ai-scientist"
  - "positioning"
...
data_products: []
algorithms:
  - name: "Trial -> evaluator -> verbal-reflection -> retry loop"
  - name: "Episodic memory of textual reflections"
  - name: "No gradient updates"
...
claim_boundary:
  scope: >-
    Bounded to the three benchmark families and the textual self-reflection prompt scheme.
  out_of_scope:
  - "Do not claim parameter-free improvement holds across all domains."
  - "Do not over-trust verbal reflection as a faithful failure model."

failure_modes:
  - "Reflection can hallucinate causes."
  - "Memory window cost grows with trials."

depends_on:
  - paper-yao-2022-react-reasoning-acting-language-models

adapter_notes: []

research_generation_affordances:
  - type: hypothesis
    statement: "HelioSI's molting + LingTai durable knowledge is a structural superset of Reflexion: it persists not just textual reflections but also paper-skill-grounded failure_modes."
  - type: minimal_experiment
    statement: "Convert each PFSS / CDAWeb failure observed in HelioSI sessions into a Reflexion-style memory entry and feed it back on the next attempt; measure retry success rate."

provenance:
  generated_by: "HelioSI paper-to-skill factory (Claude Opus 4.7) - wave500_agent_runtime_eval_design_045"
  generated_at: "2026-05-18T00:00:00Z"
  source_record: "sioulas-reproduction/results/paper_skill_corpus/wave500_agent_runtime_eval_design_045/manifest.json and sioulas-reproduction/results/agent_runtime_paper_scan_raw.md / agent_runtime_2026_only_synthesis.md (arxiv:2303.11366, doi:n/a)"
  verified_by: null
  verified_at: null
...
# Reflexion: Language Agents with Verbal Reinforcement Learning - paper-skill (wave500)
...
- designing a HelioSI benchmark or evaluation that should reflect the agent-runtime-self-reflection-loop pattern
...
**Claim (narrow form).** Reflexion adds verbal self-reflection on past failures to an LLM agent loop, improving performance on coding (HumanEval), web navigation (AlfWorld), and reasoning tasks without parameter updates.
...
### Method 1: Trial -> evaluator -> verbal-reflection -> retry loop
...
### Method 2: Episodic memory of textual reflections

- Paper role: explicit named method/component of the paper's contribution.
- Abstract procedure: the runtime that wants to borrow this pattern must be able to (a) instantiate the component as a callable, (b) feed it the manuscript/benchmark/sibling-skill inputs from §4, and (c) emit an artifact a Layer-4 affordance can reference.

### Method 3: No gradient updates
...
**In scope.** Bounded to the three benchmark families and the textual self-reflection prompt scheme.
...
- `[[paper-yao-2022-react-reasoning-acting-language-models]]` - paper-skill within the same wave500 batch (sibling positioning/design-pattern source).
...
- **Hypothesis** - HelioSI's molting + LingTai durable knowledge is a structural superset of Reflexion: it persists not just textual reflections but also paper-skill-grounded failure_modes. Related: n/a.
- **Minimal_experiment** - Convert each PFSS / CDAWeb failure observed in HelioSI sessions into a Reflexion-style memory entry and feed it back on the next attempt; measure retry success rate. Related: n/a.
