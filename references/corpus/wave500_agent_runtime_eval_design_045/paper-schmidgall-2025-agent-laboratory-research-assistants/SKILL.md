...
domain:
  primary_theme: other
  secondary_themes: ["agent-runtime", "agent-runtime-human-in-loop"]
  missions: []
  regime: []

trigger_keywords:
  - "ai-scientist-system"
  - "agent-runtime-human-in-loop"
  - "agent-runtime"
  - "ai-scientist"
  - "positioning"
...
data_products: []
algorithms:
  - name: "Three-stage pipeline: literature review -> experimentation -> report writing"
  - name: "Per-stage human feedback gating"
  - name: "Driver-LLM ablation across multiple foundation models"
  - name: "Researcher-survey + code-quality + cost evaluation"

validation_target: null

links:
  doi_url: null
  arxiv_url: "https://arxiv.org/abs/2501.04227"
  ads_url: null
  code_repo: null
  data_repo: null
...
claim_boundary:
  scope: >-
    Bounded to ML-research tasks with a researcher survey as evaluator and o1-preview as the strongest driver; cost reduction is relative to prior autonomous systems, not to human researchers.
  out_of_scope:
  - "Do not claim cost-vs-human-researcher savings; the baseline is autonomous systems."
  - "Do not treat 'human-in-loop improves quality' as evidence of autonomy."

failure_modes:
  - "Driver dependence: the headline result rests on one specific LLM; the system may regress as that model deprecates."
  - "Survey-based evaluation is small-N; not equivalent to peer review."
  - "Cost comparison excludes feedback-provider time on the human side."
...
research_generation_affordances:
  - type: hypothesis
    statement: "HelioSI's expert-in-the-loop (the HelioSI PI) can be framed as the Agent-Laboratory pattern instantiated for heliophysics, with paper-skills replacing free-form literature review."
  - type: minimal_experiment
    statement: "Re-run a heliophysics workflow (e.g. PFSS source mapping for one PSP encounter) under two regimes — PI gating each stage vs. no gating — and measure the rate of factually-incorrect intermediate claims."

provenance:
  generated_by: "HelioSI paper-to-skill factory (Claude Opus 4.7) - wave500_agent_runtime_eval_design_045"
  generated_at: "2026-05-18T00:00:00Z"
  source_record: "sioulas-reproduction/results/paper_skill_corpus/wave500_agent_runtime_eval_design_045/manifest.json and sioulas-reproduction/results/agent_runtime_paper_scan_raw.md / agent_runtime_2026_only_synthesis.md (arxiv:2501.04227, doi:n/a)"
  verified_by: null
  verified_at: null
...
# Agent Laboratory: Using LLM Agents as Research Assistants - paper-skill (wave500)
...
- designing a HelioSI benchmark or evaluation that should reflect the agent-runtime-human-in-loop pattern
...
**Claim (narrow form).** Agent Laboratory completes the literature-review -> experiment -> report-writing loop with human feedback at each stage; the o1-preview driver produces the best outputs, generates state-of-the-art ML code on the tested tasks, and cuts research expense by 84% vs. prior autonomous-research baselines.
...
### Method 1: Three-stage pipeline: literature review -> experimentation -> report writing
...
### Method 2: Per-stage human feedback gating

- Paper role: explicit named method/component of the paper's contribution.
- Abstract procedure: the runtime that wants to borrow this pattern must be able to (a) instantiate the component as a callable, (b) feed it the manuscript/benchmark/sibling-skill inputs from §4, and (c) emit an artifact a Layer-4 affordance can reference.

### Method 3: Driver-LLM ablation across multiple foundation models

- Paper role: explicit named method/component of the paper's contribution.
- Abstract procedure: the runtime that wants to borrow this pattern must be able to (a) instantiate the component as a callable, (b) feed it the manuscript/benchmark/sibling-skill inputs from §4, and (c) emit an artifact a Layer-4 affordance can reference.

### Method 4: Researcher-survey + code-quality + cost evaluation
...
**In scope.** Bounded to ML-research tasks with a researcher survey as evaluator and o1-preview as the strongest driver; cost reduction is relative to prior autonomous systems, not to human researchers.
...
- DOI: n/a
- arXiv: [arXiv:2501.04227](https://arxiv.org/abs/2501.04227)
- ADS: n/a
- Code: n/a
- Data: n/a
...
- **Hypothesis** - HelioSI's expert-in-the-loop (the HelioSI PI) can be framed as the Agent-Laboratory pattern instantiated for heliophysics, with paper-skills replacing free-form literature review. Related: n/a.
- **Minimal_experiment** - Re-run a heliophysics workflow (e.g. PFSS source mapping for one PSP encounter) under two regimes — PI gating each stage vs. no gating — and measure the rate of factually-incorrect intermediate claims. Related: n/a.
