...
domain:
  primary_theme: other
  secondary_themes: ["agent-runtime", "agent-runtime-async-tournament"]
  missions: []
  regime: []

trigger_keywords:
  - "ai-scientist-system"
  - "agent-runtime-async-tournament"
  - "agent-runtime"
  - "ai-scientist"
  - "positioning"
...
data_products: []
algorithms:
  - name: "Asynchronous multi-agent task execution"
  - name: "Generate-debate-evolve hypothesis loop"
  - name: "Tournament evolution with test-time compute scaling"
  - name: "Domain validation hand-off to in-vitro / organoid experiments"
...
claim_boundary:
  scope: >-
    Bounded to three biomedical domains (drug repurposing, novel target discovery, bacterial-evolution mechanism); validation is partial and reported in separate co-timed reports.
  out_of_scope:
  - "Do not treat 'novel hypothesis' generation as equivalent to validated scientific finding without the separate experimental reports."
  - "Do not generalize the tournament-evolution mechanism to settings where hypothesis comparability is undefined."

failure_modes:
  - "Tournament metric design: dominance under one ranking does not imply scientific superiority."
  - "Validation is partial; system credit attribution requires reading the co-timed reports."
  - "Async execution complicates reproducibility of any one hypothesis trajectory."
...
provenance:
  ...
  source_record: "sioulas-reproduction/results/paper_skill_corpus/wave500_agent_runtime_eval_design_045/manifest.json and sioulas-reproduction/results/agent_runtime_paper_scan_raw.md / agent_runtime_2026_only_synthesis.md (arxiv:2502.18864, doi:n/a)"
  verified_by: null
  verified_at: null
...
# Towards an AI co-scientist - paper-skill (wave500)
...
- designing a HelioSI benchmark or evaluation that should reflect the agent-runtime-async-tournament pattern
...
**Claim (narrow form).** A multi-agent system on Gemini 2.0 implementing a generate-debate-evolve loop with asynchronous task execution and tournament-style hypothesis evolution improves hypothesis quality under test-time compute scaling, and proposes biomedical candidates (drug repurposing for AML, anti-fibrotic targets) with partial in-vitro/organoid validation.
...
### Method 1: Asynchronous multi-agent task execution
...
### Method 2: Generate-debate-evolve hypothesis loop

- Paper role: explicit named method/component of the paper's contribution.
- Abstract procedure: the runtime that wants to borrow this pattern must be able to (a) instantiate the component as a callable, (b) feed it the manuscript/benchmark/sibling-skill inputs from §4, and (c) emit an artifact a Layer-4 affordance can reference.

### Method 3: Tournament evolution with test-time compute scaling

- Paper role: explicit named method/component of the paper's contribution.
- Abstract procedure: the runtime that wants to borrow this pattern must be able to (a) instantiate the component as a callable, (b) feed it the manuscript/benchmark/sibling-skill inputs from §4, and (c) emit an artifact a Layer-4 affordance can reference.

### Method 4: Domain validation hand-off to in-vitro / organoid experiments
...
**In scope.** Bounded to three biomedical domains (drug repurposing, novel target discovery, bacterial-evolution mechanism); validation is partial and reported in separate co-timed reports.
...
- DOI: n/a
- arXiv: [arXiv:2502.18864](https://arxiv.org/abs/2502.18864)
- ADS: n/a
- Code: n/a
- Data: n/a
