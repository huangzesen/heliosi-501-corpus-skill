...
domain:
  primary_theme: other
  secondary_themes: ["agent-runtime", "agent-runtime-agent-computer-interface"]
  missions: []
  regime: []

trigger_keywords:
  - "agent-system"
  - "agent-runtime-agent-computer-interface"
  - "agent-runtime"
  - "ai-scientist"
  - "positioning"
...
data_products: []
algorithms:
  - name: "Custom agent-computer interface (view / edit / search / run)"
  - name: "LM-friendly tool output formatting"
  - name: "Iterative agent loop with structured actions"
...
claim_boundary:
  scope: >-
    Bounded to SWE-bench-style coding tasks with the specific interface designed.
  out_of_scope:
  - "Do not over-generalize the specific interface choices to non-coding domains."
  - "Do not claim interface design always dominates model choice — the paper shows it dominates on this benchmark family."

failure_modes:
  - "Interface coupling to specific LM tokenization."
  - "Maintenance burden as the underlying tools evolve."

depends_on:
  - paper-jimenez-2024-swebench-resolving-github-issues

adapter_notes: []

research_generation_affordances:
  - type: hypothesis
    statement: "HelioSI's MCP set IS the heliophysics agent-computer interface. The lesson: design the per-MCP output schema for LM consumption, not for HTML/UI."
  - type: minimal_experiment
    statement: "Compare two CDAWeb adapter designs (raw JSON dump vs. LM-friendly summary + on-demand drill-in) on a heliophysics tool-use task; report turn count + correctness delta."

provenance:
  generated_by: "HelioSI paper-to-skill factory (Claude Opus 4.7) - wave500_agent_runtime_eval_design_045"
  generated_at: "2026-05-18T00:00:00Z"
  source_record: "sioulas-reproduction/results/paper_skill_corpus/wave500_agent_runtime_eval_design_045/manifest.json and sioulas-reproduction/results/agent_runtime_paper_scan_raw.md / agent_runtime_2026_only_synthesis.md (arxiv:2405.15793, doi:n/a)"
  verified_by: null
  verified_at: null
...
# SWE-agent: Agent-Computer Interfaces Enable Automated Software Engineering - paper-skill (wave500)
...
- designing a HelioSI benchmark or evaluation that should reflect the agent-runtime-agent-computer-interface pattern
...
**Claim (narrow form).** Designing the agent-computer interface (file viewer, edit, search, run commands tailored for LM consumption) is a first-class lever for SWE-bench performance; SWE-agent's interface design alone yields substantial gains.
...
### Method 1: Custom agent-computer interface (view / edit / search / run)
...
### Method 2: LM-friendly tool output formatting

- Paper role: explicit named method/component of the paper's contribution.
- Abstract procedure: the runtime that wants to borrow this pattern must be able to (a) instantiate the component as a callable, (b) feed it the manuscript/benchmark/sibling-skill inputs from §4, and (c) emit an artifact a Layer-4 affordance can reference.

### Method 3: Iterative agent loop with structured actions
...
**In scope.** Bounded to SWE-bench-style coding tasks with the specific interface designed.
...
- `[[paper-jimenez-2024-swebench-resolving-github-issues]]` - paper-skill within the same wave500 batch (sibling positioning/design-pattern source).
...
- **Hypothesis** - HelioSI's MCP set IS the heliophysics agent-computer interface. The lesson: design the per-MCP output schema for LM consumption, not for HTML/UI. Related: n/a.
- **Minimal_experiment** - Compare two CDAWeb adapter designs (raw JSON dump vs. LM-friendly summary + on-demand drill-in) on a heliophysics tool-use task; report turn count + correctness delta. Related: n/a.
