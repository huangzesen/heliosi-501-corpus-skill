...
|---:|---:|---|
| 1 / 4 | 87 | `wave500_instruments_data_software_045` (42; 3 archive entries flipped `research_generation_affordance: true` during the validation/Layer-4 internalization pass), `wave500_sw_classification_ml_foundation_045` (45) |
| 2 / 4 | 42 | `wave500_turbulence_intermit_heating_045` (39 after 6 entries promoted to 3/4 by the 2026-05-19 turbulence internalization batch), plus 3 `wave500_instruments_data_software_045` archive entries (PSP SOC, JSOC, SOAR) that gained `research_generation_affordance: true` |
| 3 / 4 | 96 | `wave500_agent_runtime_eval_design_045` (45), `wave500_sep_shocks_space_weather_045` (45), and `wave500_turbulence_intermit_heating_045` (6 promoted entries: Duan 2021, Cuesta 2022, Andrés 2022 [slug carries 2021], Shi 2021, Soljento 2023, McIntyre 2024 [slug; published 2025]) |
...
| `research_generation_affordance` | 138 |
...
at the time the audit was first pinned; the live count is now **98 / 501**
after (a) a turbulence-batch enrichment promoted the Sioulas 2022 manifest
title to its full journal form (which contains `Alfvén` ⟶ adds one entry
to the U+00E9 column) and (b) the 2026-05-19 corona/CME internalization
batch promoted two `wave500_solar_corona_cme_flares_045` manifest titles
to their journal/anchor form, each of which contains a U+2014 EM DASH
(`paper-gong-network-synoptic-magnetogram-product`,
`paper-eui-fsi-hri-coronal-bright-points-statistics`). The framing below is unchanged: the corpus paraphrases
...
| Metric | Value |
|---|---:|
| Entries with non-ASCII title | **98 / 501** |
...
98 / 501 entries with non-ASCII titles and the same 11 unique code points;
...
| U+2013 | – | EN DASH | 10 | typographic ranges |
| U+2014 | — | EM DASH | 34 | title separator |
...
- The 98-entry count is **expected**, not a defect. The corpus paraphrases
